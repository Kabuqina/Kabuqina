...
## 三、平台配置（Telegram / 微信 / QQ / 钉钉 / WhatsApp / Email）
...
### TC-FE-012 [P1] 钉钉 / WhatsApp / Email 配置流程

| 属性 | 内容 |
|------|------|
| **Given** | 当前 profile 对应的保留平台配置区块可见 |
| **When** | 钉钉保存 App Key/Secret；WhatsApp 完成扫码；Email 保存服务器与账号 |
| **Then** | 显示已配置状态；缺少必填字段时失败关闭且不启动错误平台 |

**手工执行步骤：**
1. mainland profile 验证钉钉必填字段与保存后重启状态。
2. SEA profile 验证 WhatsApp 二维码超时、刷新与重绑。
3. SEA profile 验证 Email 服务器、端口、账号和凭据缺失时的错误提示。
4. 切换 profile 后确认另一组平台不创建配置或启动状态。
...
**手工执行步骤：** 打开微信配置区块，生成二维码并使用微信扫描确认。

**异常场景：** 覆盖二维码超时、重复扫码和换绑。
