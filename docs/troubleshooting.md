...
## 19. Graph 运行时异常时的常规版本回退

**Symptom**

* 升级后，只有对话执行出现 Graph 运行时回归；Provider、密钥和网络均正常。
* 日志或最小复现表明问题来自当前版本，需要先恢复可用性再收集诊断信息。

**Fix**

自 v0.4.0 起，桌面和 Gateway 都只使用图运行时；没有可选的旧引擎。
遇到运行时问题时：

1. 保留桌面日志、Provider/API 格式、请求时间和最小复现步骤。
2. 暂停受影响的自动化任务，避免对可能已执行过工具调用的对话进行重放。
3. 按发布流程回退到上一个已验证安装包，或等待包含修复的新版本；不要通过
   修改会话配置或环境变量尝试切换引擎。

**Lesson**

> 版本回退恢复的是已验证的应用构建，不是同一构建中的隐藏运行时分支。保留
> 完整证据后再升级到修复版本。
