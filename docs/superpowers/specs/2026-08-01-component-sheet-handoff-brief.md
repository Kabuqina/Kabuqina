...
>
> ⚠️ **范围更新（2026-08-13）：Studio 已从产品砍掉，sheet 不再需要 Studio 物件。**
> 下文提到的 **工作夹、纵向标签、观点卡、Brief 便条、复印件、成件** 属于 Studio，**跳过它们**。
> 只做 Study 物件：书立（横向标签）、课程本、书脊/书堆、卡片盒、铅笔卡/落墨卡、书签、
> 杂记本、台灯、小娜杯、纸张三阶、线四重、状态四色、通用控件、焦点环、骨架屏。
> 令牌与对比度约束全部照旧。
...
`--veil` / `--veil-strong` 是**浮在桌面上的浅色半透明面板**（书堆盒、素材堆、窄屏工具条）。
浅色主题下必须是近白；做成压暗的一层会让上面的小字掉到 **1.83:1**，几乎读不出来。
暗色主题下才是深色纱。
...
--desk-glow:transparent; --veil:rgba(255,255,255,.78); --veil-strong:rgba(255,253,248,.96);
...
## 6.5 第一轮返工要求（2026-08-01）

第一轮交了两版。**采用「加深桌感」那版的方向**——整页即桌面、笔记做成微斜的纸片、物件按
"摊开着 / 插在架上 / 抽屉里 / 摊在桌角"归位。另一版是白页上摆卡片，桌面只作为卡内样本出现，
方向不对。在此基础上三处返工：

### 一、桌面要有"质感"，现在只有渐变

当前桌面是一条三段线性渐变，颜色对了但**没有材质**——看上去是一块色板，不是一张桌子。

补一层 **SVG 噪点**（`feTurbulence` + 极低 alpha 的 `feColorMatrix`，内联 data-URI）。
这不是新发明：最初的场景原型 `prototypes/2026-07-07-notebook-ia.html` 就是这么做的，
美术计划也明确允许 SVG 承重、只禁止**位图**纹理。建议起点：`baseFrequency≈0.9`、
`numOctaves=2`、alpha ≈ `0.02`，纸面可用更弱的一档。

**注意**：噪点属于"关掉后层级仍须成立"的装饰层，不能让任何信息依赖它。

### 二、台灯不要自己画——用产品里现成的 Lucide `LampDesk`（2026-08-01 定论）

这条已经绕了两版，就此钉死。

sheet 里那盏手绘台灯**作废**。它 `viewBox 0 0 48 48` + `stroke-width 2`，线宽比 0.042，
比产品所有图标（Lucide 24×24 + 1.7，线宽比 0.071）**细约 40%**，摆在一起显得虚飘；灯罩是
不等边四边形不是截头圆锥，光线是三条等长**平行**短线（排线，不是从一点发散的光），重心还翻
在底座之外。

而**产品里根本不需要它**：`web/src/shell/AppShell.tsx` 一直用的是 Lucide 的 `LampDesk`——
一盏带灯罩、灯臂、底座的正经台灯，24×24、与其余图标同族，`.kq-lamp-toggle.is-on` 亮时
`color:--lamp-lit` + `drop-shadow(0 0 7px --lamp-glow)`。这就是最终形态。

> **规则：台灯以 `AppShell` 的 Lucide `LampDesk` 为准。sheet 不得重画，后续会话也不要
> 重画。**若确需"亮灯态发散光线"，只在 `LampDesk` 之上叠光线、坐标与线宽跟随 Lucide，不动灯身。

**台灯按钮与桌面暖光解耦（选项 B）。**台灯开关按钮在左上品牌区（和 Logo、名称在一起），
但桌面的 `--desk-glow` 暖光**不必**从那个图标发出——按钮是「开关灯」的控件，桌面暖光是独立
的环境光，落在右上纸张浮起处，像夜里一盏看不见的灯照着桌面。所以 sheet 里"灯罩朝向桌面、
光与暖斑几何对齐"的要求**取消**。

### 三、`--veil` 取值已更正

第一轮拿到的是 `rgba(64,53,79,.28/.42)`（暗紫纱），**是错的**——那会让书堆盒上的小字掉到
1.83:1。已更正为近白（见 §3.3 与 §4）。请以本文最新值为准重出。

> **小娜杯不在本轮范围。**它是品牌 logo 级资产，另轨设计；sheet 里保留当前占位形状即可。
