...
当前 SVG 是 lucide 的 `lightbulb` 形状——一个**灯泡**。这不只是好不好看：

- 灯泡是通用的"点子 / 光"图标，桌上不存在这么一个悬空的灯泡；
- 更要紧的是，桌面的暖斑 `--desk-glow` 定义为**"台灯落在桌面上的光"**，锚在右上角。
  灯泡解释不了那束光从哪来；**有灯罩、有灯臂、有底座的台灯**才解释得了。

要求：**底座 + 灯臂（可弯折）+ 灯罩**，灯罩朝向桌面（左下方），亮着时光从灯罩口洒出、
与右上角那块 `--desk-glow` 对得上。仍用 stroke 线稿风格（与 Lucide 同族），不做写实渲染。
关：`--ink-soft` 描边；开：`--lamp-lit` + `drop-shadow(0 0 7px var(--lamp-glow))`。
