...
1. 节点已有 active core：保存 location 并进入学习页；
2. 没有 active core，但已有待采用 draft：打开该节点的草稿审核；
3. 有编译 run 正在进行：留在计划页，显示“正在整理这一节”及运行状态；
4. 没有 run：创建高优先级编译，显示状态；
5. `needs_source/failed`：显示原因和“重试 / 打开知识源 / 问小娜”。
...
| 数据状态 | 按钮/反馈 |
|---|---|
| 有 active core | `学习`，直接进入 |
| 正在编译 | `正在整理…`，不可重复提交 |
| 有 draft | `查看知识核`，打开草稿审核 |
| needs_source | `指定知识源`，并提供次级“问小娜” |
| failed | `重新整理`，显示简洁错误 |
