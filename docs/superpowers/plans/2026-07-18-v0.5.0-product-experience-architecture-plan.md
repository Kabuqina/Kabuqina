...
>
> **状态：** Draft v4（2026-07-18）。这是 v0.5.0 Study 详细 UX、全局 UI 美术和
> Chat/Study/Create 跨面实现的共同上游。Gate E4 前，不把当前 Chat 右栏或独立 Study
> 页面直接美化成最终产品结构；已冻结的 Tutor、Practice、Learning contracts 可并行推进。
>
> **本轮 plan-session 接收的 E3 seed（待正式复核）：** [Study ↔ Chat 灰阶交互原型](../prototypes/2026-07-18-v0.5.0-study-chat-low-fi.html)
> 与 [产品 UX/UI handoff §13](../handoffs/2026-07-18-v0.5.0-product-ux-ui-figma-handoff.md#13-本轮收尾记录2026-07-18)。
> 它们记录 fixture 层证据、direct full course Chat + exact return 的 baseline/fallback 与风险门；
> `CTL-A01` 仍为 `ACTIVE`。只有明确标为 `OWNER-FROZEN/FROZEN-SPEC` 的事实可直接复用，
> `TO-TEST/OPEN` 不变，仍待 accountable owner/E3/E4 验收，不等于 production sign-off。
>
> **场景权威边界：**
