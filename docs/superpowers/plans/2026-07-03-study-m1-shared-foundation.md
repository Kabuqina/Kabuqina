...
- [x] **Step 1: write failing contract + per-kind schema tests.** Cover valid
...
- [x] **Step 2: define the contract.** Envelope `version = 1`; lifecycle statuses
...
- [x] **Step 3: verify + commit.** `pytest tests/learning/test_learning_contract.py`.
...
- [x] **Step 1: write failing store + isolation tests.** Assert: a fresh db
...
- [x] **Step 2: implement the store + context.** Separate `learning.db` under the
  ...
  artifacts.
- [x] **Step 3: prove concurrency + isolation.** Two connections (web-child /
...
- [x] **Step 1: write failing writer tests.** Envelope + per-kind payload
  ...
  bridge is M2).
- [x] **Step 2: implement.** Validate against Task 1's schemas; inject owner from
...
- [x] **Step 1: write failing index tests.** `learning_index_build` is
...
- [x] **Step 2: implement the minimal skeleton.** Read-only over Task 2's store;
...
- [x] **Step 1: write the failing equivalence test first (critical).** Assert the
...
- [x] **Step 2: define `PlannerSpec` + registry.** `PlannerSpec` declares only:
...
- [x] **Step 3: verify both prompt families + commit.** Run the equivalence test
...
- [x] **Step 1: write failing toolset tests.** The M1 subset — list/create/select
...
- [x] **Step 2: implement + register.** Thin tools delegating to Tasks 2–4;
...
- [x] **Step 1: write the failing drift test.** The registry references the new
  ...
  entries.
- [x] **Step 2: add references.** Append learning pipeline entries to the
...
- [x] **Step 1: write failing tests.** A stable local desktop `owner_id` is
...
- [x] **Step 2: implement injection.** Desktop owner id + wiring into the context
...
python -m pytest tests/agent/test_prompt_builder.py -o "addopts=" -p no:cacheprovider -q
cd ..\python
python -m pytest tests/test_capability_registry_learning.py tests/test_learning_owner_context.py tests/test_policy_contract.py::TestToolPolicy -o "addopts=" -p no:cacheprovider -q
```

**Acceptance (design §13 M1):**

- [x] Existing PPT / PDF / HTML / DOCX Planner behaviour is unchanged (the
  equivalence test + `test_prompt_builder.py` are green).
- [x] Two children (web-child / gateway-child shape) read and write **isolated**
  course spaces under **different owners** safely (store + context + owner-injection
  tests green).
- [x] `capability_registry` validation + drift tests green; no id duplication.
- [x] No agent-engine files touched; shared-file additions are append-only.

**Closure evidence (2026-07-04):**

- Review remediation closed the keep-list gap (`learning` is enabled in desktop
  and gateway policy), the candidate lifecycle field (`lifecycle: candidate`),
  the no-selected-space `learning_artifact_list` tool-error path, and full
  envelope JSON/size validation for `source_refs`.
- Fresh M1 gate from `hermes_core`: 256 passed, 1 skipped
  (`python -m pytest tests/learning tests/agent/test_prompt_builder.py -o "addopts=" -p no:cacheprovider -q`).
- Fresh desk gate from `python`: 25 passed
  (`python -m pytest tests/test_capability_registry_learning.py tests/test_learning_owner_context.py tests/test_policy_contract.py::TestToolPolicy -o "addopts=" -p no:cacheprovider -q`).
