...
无论它们以独立文件还是内联代码形式存在）。美术母版保存在私有仓库
`Kabuqina/kabuqina-mascot` 中；本分支（`stu-competition`）在构建路径
（`web/public/`、`tauri/icons/`）保留了真实品牌资产的分发副本，这些副本
仍为专有授权，不是 Apache-2.0 占位资产。历史 `Na_logo/` 树虽已退追踪，
仍受授权覆盖。拆分细则见 [BRAND.md](BRAND.md)。
