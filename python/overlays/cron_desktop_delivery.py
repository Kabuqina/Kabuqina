...
Tauri desktop_delivery bridge. Any non-local target (weixin, qqbot, …) is
...
def _resolve_desktop_targets(deliver_value: str) -> tuple[bool, str]:
    ...
    Rules:
      ...
      - ``"desktop, weixin:wxid"``           -> desktop AND weixin (return ``"weixin:wxid"``)
      - ``"weixin:wxid"``                    -> no desktop, full string forwarded
    """
