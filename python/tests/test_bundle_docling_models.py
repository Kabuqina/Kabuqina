...
class BundlePresenceTests(unittest.TestCase):
    ...
    def test_bundle_script_copies_learning_core_package(self):
        root = Path(__file__).resolve().parents[1]
        script = (root / "build_bundle.ps1").read_text(encoding="utf-8")
        sync_script = (root.parent / "scripts" / "sync-runtime-sources.ps1").read_text(encoding="utf-8")

        self.assertIn('"learning"', script)
        self.assertIn('"learning"', sync_script)

    def test_bundle_script_copies_learning_owner_policy(self):
        root = Path(__file__).resolve().parents[1]
        script = (root / "build_bundle.ps1").read_text(encoding="utf-8")
        sync_script = (root.parent / "scripts" / "sync-runtime-sources.ps1").read_text(encoding="utf-8")

        self.assertIn("src\\learning_owner.py", script)
        self.assertIn('"learning_owner.py"', sync_script)

    def test_build_bundle_skips_docling_model_bundling_by_default(self):
        root = Path(__file__).resolve().parents[1]
        script = (root / "build_bundle.ps1").read_text(encoding="utf-8")

        self.assertIn("Docling base models are load-packages", script)
        self.assertIn("Skipping Docling model bundling by default", script)
        self.assertNotIn("Bundling Docling models (layout + table + EasyOCR; CodeFormula excluded)", script)
