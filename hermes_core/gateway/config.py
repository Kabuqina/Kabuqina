...
class Platform(Enum):
    ...
    Removed platform values remain explicit compatibility tokens so persisted
    jobs/config can be read and rejected without rerouting. Product platform
    plugins are no longer supported and arbitrary values are rejected.
    """
    LOCAL = "local"
    TELEGRAM = "telegram"
    DISCORD = "discord"
...
_REMOVED_PLATFORM_VALUES = frozenset({
    "discord",
    "feishu",
    "wecom",
    "wecom_callback",
...
# retained token-based adapters do not need an entry here.
...
class GatewayConfig:
    ...
    # Storage paths
    sessions_dir: Path = field(default_factory=lambda: get_kabuqina_home() / "sessions")
    ...
    thread_sessions_per_user: bool = False  # Share thread/topic sessions by default; set true to isolate per participant
    ...
    def _is_platform_connected(self, platform: Platform, config: PlatformConfig) -> bool:
        ...
        if platform.value in _REMOVED_PLATFORM_VALUES:
            return False
        ...
        # Generic token/api_key auth covers retained token-based adapters.
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "GatewayConfig":
        ...
        for platform_name, platform_data in data.get("platforms", {}).items():
            if platform_name in _REMOVED_PLATFORM_VALUES:
        ...
        sessions_dir = get_kabuqina_home() / "sessions"
...
def load_gateway_config() -> GatewayConfig:
    ...
    2. ~/.kabuqina/config.yaml (primary user-facing config)
    3. ~/.kabuqina/gateway.json (legacy — provides defaults under config.yaml)
    4. Built-in defaults
    """
        ...
        if config_yaml_path.exists():
            ...
            if isinstance(yaml_platforms, dict):
                for plat_name, plat_block in yaml_platforms.items():
                    if plat_name in _REMOVED_PLATFORM_VALUES:
                        continue
            ...
            for plat in Platform:
                if plat == Platform.LOCAL or plat.value in _REMOVED_PLATFORM_VALUES:
                    continue
