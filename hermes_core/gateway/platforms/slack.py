...
class SlackAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        tokens_file = get_kabuqina_home() / "slack_tokens.json"
        ...
        try:
            ...
            # /model work directly without /kabuqina prefix). A single regex
            ...
            # manifest (see `kabuqina slack manifest`). In Socket Mode, Slack
            ...
            from kabuqina_cli.commands import slack_native_slashes
            import re as _re
            ...
            if _slash_names:
                ...
                _slash_pattern = _re.compile(r"^/kabuqina$")

            @self._app.command(_slash_pattern)
            async def handle_kabuqina_command(ack, command):
                await ack()
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own Kabuqina session — matching the per-thread behavior channels already
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        ``/kabuqina`` is canonical. The legacy ``/hermes <subcommand> [args]``
        form is preserved for
        ...
        if slash_name in ("kabuqina", "hermes", ""):
            # Legacy /hermes <subcommand> [args] routing + free-form questions.
            # Empty slash_name falls into this branch for backward compat
            # with any caller that didn't populate command["command"].
            from kabuqina_cli.commands import slack_subcommand_map
