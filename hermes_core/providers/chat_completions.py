...
Resolution order for text tasks (auto mode):
  ...
  3. Nous Portal (~/.kabuqina/auth.json active provider)
...
NOUS_EXTRA_BODY = {"tags": ["product=kabuqina-agent"]}
...
_AUTH_JSON_PATH = get_kabuqina_home() / "auth.json"
...
class _AnthropicCompletionsAdapter:
    ...
    def create(self, **kwargs) -> Any:
        ...
        if temperature is not None:
            from providers.anthropic import _forbids_sampling_params
...
def _endpoint_speaks_anthropic_messages(base_url: str) -> bool:
    ...
    Mirrors ``kabuqina_cli.runtime_provider._detect_api_mode_for_url`` so the
...
def _maybe_wrap_anthropic(
    ...
    try:
        from providers.gemini import GeminiNativeClient
...
def _read_nous_auth() -> Optional[dict]:
    """Read and validate ~/.kabuqina/auth.json for an active Nous provider.
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    """
    try:
        from kabuqina_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if provider_id == "anthropic":
            ...
            try:
                from kabuqina_cli.auth import is_provider_explicitly_configured
...
def _try_nous(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    model = _NOUS_MODEL
    try:
        from kabuqina_cli.models import get_nous_recommended_aux_model
...
def _validate_base_url(base_url: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run `kabuqina setup` or `kabuqina model` and enter a valid http(s) base URL."
        ) from exc
...
def _try_anthropic() -> Tuple[Optional[Any], Optional[str]]:
    ...
    # base_url would leak into Anthropic requests.
...
def _get_provider_chain() -> List[tuple]:
    ...
    Kabuqina gateway children use the user's configured provider only;
...
def _refresh_provider_credentials(provider: str) -> bool:
    ...
    try:
        ...
        if normalized == "anthropic":
            from providers.anthropic import read_claude_code_credentials, _refresh_oauth_token, resolve_anthropic_token
...
def _resolve_auto(main_runtime: Optional[Dict[str, Any]] = None) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    #    scenario where a user switches providers via `kabuqina model` but the
    #    old OPENAI_BASE_URL lingers in ~/.kabuqina/.env. ──
    if not _stale_base_url_warned:
        ...
        if (_env_base and _cfg_provider
            ...
            logger.warning(
                ...
                "Run: kabuqina model to reconfigure, or remove "
                "OPENAI_BASE_URL from ~/.kabuqina/.env",
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    try:
        from providers.gemini import GeminiNativeClient, AsyncGeminiNativeClient
...
def resolve_provider_client(
    ...
    Args:
        ...
        api_mode: API mode override.  One of "chat_completions",
            "anthropic_messages", or None (auto-detect).
    ...
    def _wrap_if_needed(client_obj, final_model_str: str, base_url_str: str = "",
        ...
        Handles ``AnthropicAuxiliaryClient`` when the endpoint speaks Anthropic
    ...
    if provider == "nous":
        ...
        if client is None:
            logger.warning("resolve_provider_client: nous requested "
                           "but Nous Portal not configured (run: kabuqina auth)")
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
                else (client, final_model))
    ...
    if provider == "custom":
        ...
        # Try custom first, then API-key providers.
    ...
    try:
        from kabuqina_cli.runtime_provider import _get_named_custom_provider
    ...
    try:
        ...
        logger.debug("kabuqina_cli.auth not available for provider %s", provider)
        return None, None
    ...
    if pconfig.auth_type == "api_key":
        ...
        # Honor api_mode for any API-key provider. Rewrap Anthropic-wire
        # endpoints (Kimi Coding Plan api.kimi.com/coding, /anthropic-suffixed
        # gateways) so named providers like kimi-coding
    ...
    if pconfig.auth_type == "external_process":
        ...
        logger.warning("resolve_provider_client: aws_sdk provider %s not "
                       "directly supported", provider)
        return None, None
...
def get_async_text_auxiliary_client(task: str = "", *, main_runtime: Optional[Dict[str, Any]] = None):
    ...
    For standard providers returns (AsyncOpenAI, model).
...
def _configured_secret(*names: str) -> bool:
    try:
        from kabuqina_cli.auth import has_usable_secret
...
def _api_key_provider_configured(provider: str) -> bool:
    if _pool_has_runtime_key(provider):
        return True
    try:
        from kabuqina_cli.auth import PROVIDER_REGISTRY, get_api_key_provider_status
...
def auxiliary_max_tokens_param(value: int) -> dict:
    ...
    Other OpenAI-compatible endpoints continue to use max_tokens.
    """
...
def _resolve_task_provider_model(
    ...
    "chat_completions", "anthropic_messages", or None (auto-detect).
    """
    cfg_provider = None
    cfg_model = None
    cfg_base_url = None
...
def _build_call_kwargs(
    ...
    if provider == "nous" or auxiliary_is_nous:
        merged_extra.setdefault("tags", []).extend(["product=kabuqina-agent"])
    if merged_extra:
...
def _validate_llm_response(response: Any, task: str = None) -> Any:
    ...
    # Allow SimpleNamespace responses from adapters (e.g.
...
def call_llm(
    ...
    *,
    ...
    try:
        ...
        # refused, timeout), try alternative providers.  This handles
        # providers the user never configured that got picked up by the
