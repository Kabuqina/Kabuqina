...
(3 SDK retries x 3 Kabuqina retries), and every one of those calls counts
...
def _state_path() -> str:
    ...
    try:
        ...
        base = os.path.join(os.path.expanduser("~"), ".kabuqina")
...
def is_genuine_nous_rate_limit(
    *,
    ...
    MiMo, Kabuqina, ...) behind one endpoint.  A 429 can mean either:
...
def _has_exhausted_bucket_in_object(state: Any) -> bool:
    ...
    Accepts the dataclass from ``providers.rate_limit_tracker`` (buckets
