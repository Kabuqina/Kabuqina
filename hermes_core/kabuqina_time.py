"""
Timezone-aware clock for Kabuqina.

Provides a single ``now()`` helper that returns a timezone-aware datetime
based on the user's configured IANA timezone (e.g. ``Asia/Kolkata``).

Resolution order:
  1. ``KABUQINA_TIMEZONE`` environment variable
  2. deprecated ``HERMES_TIMEZONE`` environment variable
  3. ``timezone`` key in ``~/.kabuqina/config.yaml``
  3. Falls back to the server's local time (``datetime.now().astimezone()``)

Invalid timezone values log a warning and fall back safely — Kabuqina never
...
try:
    ...
    # Python 3.8 fallback (the bundled Kabuqina runtime is newer).
...
def _resolve_timezone_name() -> str:
    ...
    if "KABUQINA_TIMEZONE" in os.environ:
        tz_env = os.environ["KABUQINA_TIMEZONE"].strip()
    else:
