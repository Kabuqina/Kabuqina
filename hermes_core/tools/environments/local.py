...
# Kabuqina-internal env vars that should NOT leak into terminal subprocesses.
...
def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    """Filter Kabuqina-managed secrets from a subprocess environment."""
    try:
...
def _find_bash() -> str:
    ...
    # up C:\Windows\System32\bash.exe (WSL stub), which cannot source Kabuqina
    ...
    if comspec and os.path.isfile(comspec):
        return comspec
    raise RuntimeError(
        "Git Bash not found. Kabuqina requires Git for Windows on Windows.\n"
...
def _resolve_shell_init_files() -> list[str]:
    ...
    an explicit list — once they have, Kabuqina trusts them.
    """
...
class LocalEnvironment(BaseEnvironment):
    ...
    def _run_bash(self, cmd_string: str, *, login: bool = False,
        ...
        if not _IS_WINDOWS:
            try:
                proc._kabuqina_pgid = os.getpgid(proc.pid)
        ...
        try:
            if _IS_WINDOWS:
                ...
                try:
                    ...
                    pgid = getattr(proc, "_kabuqina_pgid", None)
                    if pgid is None:
                        raise
