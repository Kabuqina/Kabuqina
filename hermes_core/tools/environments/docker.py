...
def _load_kabuqina_env_vars() -> dict[str, str]:
    """Load ~/.kabuqina/.env values without failing Docker command execution."""
    try:
        from kabuqina_cli.config import load_env
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.kabuqina/sandboxes/). Non-persistent
        ...
        container_name = f"kabuqina-{uuid.uuid4().hex[:8]}"
        run_cmd = [
    ...
    def _build_init_env_args(self) -> list[str]:
        ...
        # win over the generic Kabuqina secret blocklist. Only implicit passthrough
        # keys are filtered.
        forward_keys = explicit_forward_keys | (passthrough_keys - _HERMES_PROVIDER_ENV_BLOCKLIST)
        kabuqina_env = _load_kabuqina_env_vars() if forward_keys else {}
        for key in sorted(forward_keys):
            value = os.getenv(key)
            if value is None:
                value = kabuqina_env.get(key)
