...
def iter_sync_files(container_base: str = "/root/.kabuqina") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.kabuqina to *container_base* because the remote
    ...
    for entry in get_credential_file_mounts():
        remote = entry["container_path"].replace(
            "/root/.kabuqina", container_base, 1
        )
...
def unique_parent_dirs(files: list[tuple[str, str]]) -> list[str]:
    ...
    return sorted({
        posixpath.dirname((remote.replace("\\", "/").rstrip("/") or "/")) or "/"
...
class FileSyncManager:
    ...
    def sync_back(
        self,
        kabuqina_home: Path | None = None,
        hermes_home: Path | None = None,
    ) -> None:
        """Pull remote changes back to the host filesystem.

        Downloads the remote ``.kabuqina/`` directory as a tar archive,
        ...
        # against an uninitialized remote .kabuqina/ directory.
        if not self._pushed_hashes and not self._synced_files:
            logger.debug("sync_back: no prior push state — skipping")
            return

        # ``hermes_home`` is a one-release keyword compatibility seam.
        lock_path = (kabuqina_home or hermes_home or get_kabuqina_home()) / ".sync.lock"
    ...
    def _sync_back_impl(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="kabuqina-sync-download-") as download_dir:
            ...
            with tempfile.TemporaryDirectory(prefix="kabuqina-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.kabuqina/skills/a.md`` →
        ``~/.kabuqina/skills/a.md``, a new remote file at
        ``/root/.kabuqina/skills/b.md`` maps to ``~/.kabuqina/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
        for host, remote in mapping:
