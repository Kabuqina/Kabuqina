...
Nous, native Anthropic, or a custom OpenAI-compatible endpoint.
...
def _resolve_download_timeout() -> float:
    ...
    if env_val:
        try:
            ...
            pass
    try:
        from kabuqina_cli.config import cfg_get, load_config
        cfg = load_config()
...
async def vision_analyze_tool(
    ...
    try:
        ...
        if local_path.is_file():
            ...
            temp_dir = get_kabuqina_dir("cache/vision", "temp_vision_images")
...
if __name__ == "__main__":
    ...
    if not api_available:
        ...
        print("Configure a supported multimodal backend (OpenRouter, Nous, Anthropic, or a custom OpenAI-compatible endpoint).")
        exit(1)
    else:
