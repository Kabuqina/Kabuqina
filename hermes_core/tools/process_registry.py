...
CHECKPOINT_PATH = get_kabuqina_home() / "processes.json"
...
class ProcessRegistry:
    ...
    def spawn_via_env(
        ...
        log_path = f"{temp_dir}/kabuqina_bg_{session.id}.log"
        pid_path = f"{temp_dir}/kabuqina_bg_{session.id}.pid"
        exit_path = f"{temp_dir}/kabuqina_bg_{session.id}.exit"
    ...
    def _reconcile_local_exit(self, session: "ProcessSession") -> None:
        ...
        a daemon spawned by `kabuqina update` restarting the gateway) is still
        holding the stdout pipe open, the reader blocks forever and poll()
        keeps returning "running" indefinitely (issue #17327).
