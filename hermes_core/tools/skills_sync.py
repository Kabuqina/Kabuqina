...
Copies bundled skills from the repo's skills/ directory into ~/.kabuqina/skills/
...
The manifest lives at ~/.kabuqina/skills/.bundled_manifest.
"""
...
def _discover_bundled_skills(bundled_dir: Path) -> List[Tuple[str, Path]]:
    ...
    for skill_md in bundled_dir.rglob("SKILL.md"):
        rel_parts = skill_md.relative_to(bundled_dir).parts
        if any(part in {".git", ".github", ".hub"} for part in rel_parts):
            continue
...
def _compute_relative_dest(skill_dir: Path, bundled_dir: Path) -> Path:
    ...
    e.g., bundled/skills/mlops/axolotl -> ~/.kabuqina/skills/mlops/axolotl
    """
...
def sync_skills(quiet: bool = False) -> dict:
    """
    Sync bundled skills into ~/.kabuqina/skills/ using the manifest.
                ...
                if dest.exists():
                    ...
                    if _dir_hash(dest) == bundled_hash:
                        ...
                        print(
                            ...
                            f"was kept. Run `kabuqina skills reset {skill_name}` "
...
def reset_bundled_skill(name: str, restore: bool = False) -> dict:
    ...
    if not in_manifest and not is_bundled:
        return {
            ...
            "message": (
                ...
                f"(Hub-installed skills use `kabuqina skills uninstall`.)"
            ),
            "synced": None,
        }
    ...
    if restore and deleted_user_copy:
        ...
        action = "manifest_cleared"
        message = (
            f"Cleared manifest entry for '{name}'. Future `kabuqina update` runs "
...
if __name__ == "__main__":
    print("Syncing bundled skills into ~/.kabuqina/skills/ ...")
