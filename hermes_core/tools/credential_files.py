...
def _resolve_kabuqina_home() -> Path:
    from kabuqina_constants import get_kabuqina_home
    return get_kabuqina_home()


def _container_path(container_base: str, *parts: object) -> str:
    base = container_base.rstrip("/")
    suffix = "/".join(str(part).replace("\\", "/").strip("/") for part in parts if str(part))
    return f"{base}/{suffix}" if suffix else base


def register_credential_file(
    relative_path: str,
    container_base: str = "/root/.kabuqina",
) -> bool:
    ...
    *relative_path* is relative to ``HERMES_HOME`` (e.g. ``google_token.json``).
    ...
    """
    kabuqina_home = _resolve_kabuqina_home()
    ...
    host_path = kabuqina_home / relative_path
    ...
    containment_error = validate_within_dir(host_path, kabuqina_home)
            ...
            for item in cred_files:
                if isinstance(item, str) and item.strip():
                    ...
                    if resolved_path.is_file():
                        container_path = _container_path("/root/.kabuqina", rel)
                        result.append({
...
def get_skills_directory_mount(
    ...
    **Security:** Bind mounts follow symlinks, so a malicious symlink inside
    ...
    if skills_dir.is_dir():
        ...
        mounts.append({
            "host_path": host_path,
            "container_path": _container_path(container_base, "skills"),
        })
    ...
    try:
        ...
        for idx, ext_dir in enumerate(get_external_skills_dirs()):
            if ext_dir.is_dir():
                ...
                mounts.append({
                    "host_path": host_path,
                    "container_path": _container_path(container_base, "external_skills", idx),
                })
    except ImportError:
        pass
...
def _safe_skills_path(skills_dir: Path) -> str:
    ...
    safe_dir = Path(tempfile.mkdtemp(prefix="kabuqina-skills-safe-"))
...
def iter_skills_files(
    ...
    skills_dir = kabuqina_home / "skills"
    if skills_dir.is_dir():
        container_root = _container_path(container_base, "skills")
        for item in skills_dir.rglob("*"):
    ...
    try:
        ...
        for idx, ext_dir in enumerate(get_external_skills_dirs()):
            if not ext_dir.is_dir():
                continue
            container_root = _container_path(container_base, "external_skills", idx)
...
# Each tuple is (new_subpath, old_name) matching kabuqina_constants.get_kabuqina_dir().
...
def get_cache_directory_mounts(
    ...
    ``get_kabuqina_dir()`` for backward compatibility with old directory layouts.
    """
    from kabuqina_constants import get_kabuqina_dir

    mounts: List[Dict[str, str]] = []
    for new_subpath, old_name in _CACHE_DIRS:
        host_dir = get_kabuqina_dir(new_subpath, old_name)
        if host_dir.is_dir():
...
def iter_cache_files(
    ...
    for new_subpath, old_name in _CACHE_DIRS:
        ...
        if not host_dir.is_dir():
            continue
        container_root = _container_path(container_base, new_subpath)
