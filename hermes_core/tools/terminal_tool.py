...
- Persistent filesystems do NOT guarantee the same live sandbox or long-running processes survive cleanup, idle reaping, or Kabuqina exit
...
# display_kabuqina_home imported lazily at call site (stale-module safety during kabuqina update)
...
def _check_disk_usage_warning():
    ...
    try:
        ...
        # Get total size of current and one-release legacy directories.
        total_bytes = 0
        import glob
        paths = glob.glob(str(scratch_dir / "kabuqina-*"))
        paths += glob.glob(str(scratch_dir / "hermes-*"))
        for path in paths:
...
def _handle_sudo_failure(output: str, env_type: str) -> str:
    ...
    for failure in sudo_failures:
        if failure in output:
            from kabuqina_constants import display_kabuqina_home as _dhh
...
For servers/watchers, do NOT use shell-level background wrappers (nohup/disown/setsid/trailing '&') in foreground mode. Use background=true so Kabuqina can track lifecycle and output.
...
def _parse_env_var(name: str, default: str, converter=int, type_label: str = "integer"):
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Check ~/.kabuqina/.env or environment variables."
        )
        ...
        if modal_state["selected_backend"] != "direct":
            if modal_state["managed_mode_blocked"]:
                raise ValueError(
                    ...
                    "Modal credentials/config were found. Log in with `kabuqina model` or "
...
def _foreground_background_guidance(command: str) -> str | None:
    ...
    if _SHELL_LEVEL_BACKGROUND_RE.search(command):
        return (
            ...
            "Use terminal(background=true) so Kabuqina can track the process, then run "
...
def terminal_tool(
    ...
    try:
        ...
        if background:
            ...
            try:
                from kabuqina_cli.plugins import invoke_hook
            ...
            if modal_state["selected_backend"] != "direct":
                if modal_state["managed_mode_blocked"]:
                    logger.error(
                        ...
                        "a paid Nous subscription is required for the Tool Gateway and no direct "
                        "Modal credentials/config were found. Log in with `kabuqina model` "
