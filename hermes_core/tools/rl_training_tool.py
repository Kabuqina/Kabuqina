...
# Path to tinker-atropos submodule (relative to kabuqina-agent root)
...
LOGS_DIR = get_kabuqina_home() / "logs" / "rl_training"
...
async def rl_start_training() -> str:
    ...
    if not os.getenv("TINKER_API_KEY"):
        return json.dumps({
            "error": "TINKER_API_KEY not set. Add it to ~/.kabuqina/.env",
        }, indent=2)
