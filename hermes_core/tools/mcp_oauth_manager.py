...
`mcp_tool.py`, and `kabuqina_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# KabuqinaMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_kabuqina_provider_class() -> Optional[type]:
    ...
    class KabuqinaMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(self, *args: Any, server_name: str = "", **kwargs: Any):
            ...
            self._kabuqina_server_name = server_name
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`KabuqinaTokenStorage` persisting an absolute
            ...
            if (
                ...
                try:
                    ...
                    logger.debug(
                        ...
                        "failed (non-fatal): %s",
                        self._kabuqina_server_name, exc,
                    )
                ...
                for url in build_protected_resource_metadata_discovery_urls(
                    ...
                    try:
                        ...
                        logger.debug(
                            ...
                            self._kabuqina_server_name, url, exc,
                        )
                        continue
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        logger.debug(
                            ...
                            "token_endpoint=%s",
                            self._kabuqina_server_name, asm.token_endpoint,
                        )
                        break
    ...
    return KabuqinaMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_KABUQINA_PROVIDER_CLS: Optional[type] = _make_kabuqina_provider_class()

# Deprecated aliases for one release.
_make_hermes_provider_class = _make_kabuqina_provider_class
_HERMES_PROVIDER_CLS = _KABUQINA_PROVIDER_CLS
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Constructs :class:`KabuqinaMCPOAuthProvider` directly using the helpers
        ...
        Returns None if the MCP SDK's OAuth support is unavailable.
        """
        if _KABUQINA_PROVIDER_CLS is None:
            logger.warning(
        ...
        storage = KabuqinaTokenStorage(server_name)
    ...
    def remove(self, server_name: str) -> None:
        ...
        Called by ``kabuqina mcp remove <name>`` and (indirectly) by
        ``kabuqina mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
