...
def _kabuqina_bin_dir() -> str:
    """Return $HERMES_HOME/bin, creating it if needed."""
    d = os.path.join(_get_kabuqina_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    ...
    try:
        ...
        dest = os.path.join(_kabuqina_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    kabuqina_bin = os.path.join(_kabuqina_bin_dir(), "tirith")
    if os.path.isfile(kabuqina_bin) and os.access(kabuqina_bin, os.X_OK):
        _resolved_path = kabuqina_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return kabuqina_bin
