...
Each profile has its own `.env` file. Configure distinct retained-platform credentials in each:
...
If two profiles accidentally use the same retained-platform credential, the second gateway is blocked with a clear error naming the conflicting profile. This applies to the configured Weixin, QQBot, DingTalk, Telegram, WhatsApp, and Email identities.
