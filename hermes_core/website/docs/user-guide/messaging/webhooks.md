...
| `deliver` | No | Where to send the response: `github_comment`, `telegram`, `slack`, `signal`, `sms`, `whatsapp`, `matrix`, `mattermost`, `homeassistant`, `email`, `dingtalk`, `weixin`, `bluebubbles`, `qqbot`, or `log` (default). |
