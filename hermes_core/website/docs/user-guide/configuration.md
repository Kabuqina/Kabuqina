...
```yaml
agent:
  reasoning_effort: "high"   # default. Options: none, minimal, low, medium, high, xhigh (max)
```

The default reasoning effort is "high" so first-run agents have stronger reasoning for coding and math tasks. Lower it if you prefer fewer tokens and lower latency; higher reasoning effort gives better results on complex tasks at the cost of more tokens and latency.
...
Platforms without an override fall back to the global `tool_progress` value. Valid platform keys: `telegram`, `slack`, `signal`, `whatsapp`, `matrix`, `mattermost`, `email`, `sms`, `homeassistant`, `dingtalk`, `weixin`, `bluebubbles`, `qqbot`. The legacy `display.tool_progress_overrides` key still loads for backward compatibility but is deprecated and migrated into `display.platforms` on first load.
