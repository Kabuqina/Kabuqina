...
def test_random_strategy_uses_random_choice(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("providers.credential_pool.random.choice", lambda entries: entries[-1])
...
def test_load_pool_removes_stale_file_backed_singleton_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "providers.anthropic.read_hermes_oauth_credentials",
        lambda: None,
    )
    monkeypatch.setattr(
        "providers.anthropic.read_claude_code_credentials",
        lambda: None,
    )
...
def test_least_used_strategy_selects_lowest_count(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "providers.credential_pool._seed_from_singletons",
        lambda provider, entries: (False, set()),
    )
    monkeypatch.setattr(
        "providers.credential_pool._seed_from_env",
        lambda provider, entries: (False, set()),
    )
    _write_auth_store(
        tmp_path,
...
def test_get_custom_provider_pool_key(tmp_path, monkeypatch):
    ...
    from providers.credential_pool import get_custom_provider_pool_key
...
def test_list_custom_pool_providers(tmp_path, monkeypatch):
    ...
    from providers.credential_pool import list_custom_pool_providers
...
class TestLeastUsedStrategy:
    ...
    def test_request_count_increments(self):
        ...
        from providers.credential_pool import CredentialPool, PooledCredential, STRATEGY_LEAST_USED
        ...
        with _patch("providers.credential_pool.get_pool_strategy", return_value=STRATEGY_LEAST_USED):
