...
def hermes_home(monkeypatch):
    """Isolate KABUQINA_HOME for ``reload_skills`` tests.
    ...
    module-level ``KABUQINA_HOME`` / ``SKILLS_DIR`` constants in place so the
    isolation is local to this fixture's scope.
    """
    td = tempfile.mkdtemp(prefix="hermes-reload-skills-")
    monkeypatch.setenv("KABUQINA_HOME", td)
    home = Path(td)
    ...
    monkeypatch.setattr(_st, "KABUQINA_HOME", home, raising=False)
