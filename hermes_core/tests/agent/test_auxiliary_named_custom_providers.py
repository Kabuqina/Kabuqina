...
def _isolate(tmp_path, monkeypatch):
    """Redirect KABUQINA_HOME and clear module caches."""
...
def _write_config(tmp_path, config_dict):
    """Write a config.yaml to the test KABUQINA_HOME."""
    import yaml
...
class TestNormalizeVisionProvider:
    ...
    def test_main_resolves_to_named_custom(self, tmp_path):
        ...
        from providers.chat_completions import _normalize_vision_provider
...
class TestResolveProviderClientModelNormalization:
    ...
    def test_matching_native_prefix_is_stripped_for_main_provider(self, tmp_path):
        ...
        with (
            patch("kabuqina_cli.auth.resolve_api_key_provider_credentials", return_value={
                "api_key": "glm-key",
    ...
    def test_aggregator_vendor_slug_is_preserved(self, monkeypatch):
        ...
        with patch("providers.chat_completions.OpenAI") as mock_openai:
            mock_openai.return_value = MagicMock()
            from providers.chat_completions import resolve_provider_client
...
class TestVisionPathApiMode:
    ...
    def test_explicit_provider_passes_api_mode(self, tmp_path):
        ...
        with patch("providers.chat_completions._get_cached_client") as mock_gcc:
