...
def _build_runner(monkeypatch, tmp_path) -> GatewayRunner:
    ...
    monkeypatch.setattr(gateway_run, "_kabuqina_home", tmp_path)
...
async def test_non_internal_event_without_user_triggers_pairing(monkeypatch, tmp_path):
    ...
    # import time from whichever KABUQINA_HOME was set then. Per-test
    # KABUQINA_HOME redirection in conftest doesn't retroactively move it.
