"""
Tests for timezone support (kabuqina_time module + integration points).
...
def _reset_kabuqina_time_cache():
    """Reset the kabuqina_time module cache (replacement for removed reset_cache)."""
...
class TestHermesTimeNow:
    ...
    def test_canonical_timezone_wins_over_legacy(self):
        os.environ["KABUQINA_TIMEZONE"] = "UTC"
        os.environ["HERMES_TIMEZONE"] = "Asia/Kolkata"
        result = kabuqina_time.now()
    ...
    def test_invalid_timezone_falls_back(self, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="kabuqina_time"):
            result = kabuqina_time.now()
    ...
    def test_cache_invalidation(self):
        ...
        os.environ["HERMES_TIMEZONE"] = "UTC"
        _reset_kabuqina_time_cache()
        r1 = kabuqina_time.now()
        assert r1.utcoffset() == timedelta(0)

        os.environ["HERMES_TIMEZONE"] = "Asia/Kolkata"
        _reset_kabuqina_time_cache()
        r2 = kabuqina_time.now()
...
class TestCronTimezone:
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        ...
        # of the naive timestamp exceeds _kabuqina_now's wall time — this would
