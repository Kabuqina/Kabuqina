...
def test_activate_plan_archives_previous_and_materializes_stable_items(ctx):
    ...
    assert [row["artifact_id"] for row in service.list_plans(status="active")] == [
        second
    ]
    archived = ctx.list_artifacts(kind="learning_plan", status="archived")
    assert [row["artifact_id"] for row in archived] == [first]
    assert ctx.get_artifact(first)["review"]["status"] == "passed"
    assert ctx.get_artifact(second)["review"]["status"] == "passed"
...
def test_complete_and_skip_record_one_real_activity_each(ctx):
    ...
    assert [row["activity_type"] for row in ctx.list_activities()] == [
        PLAN_ITEM_COMPLETE_ACTIVITY,
        PLAN_ITEM_SKIP_ACTIVITY,
    ]
...
def test_reject_plan_does_not_materialize_items_and_marks_review_failed(ctx):
    service = _service(ctx)
    artifact_id = _draft(ctx)

    assert service.reject_plan(artifact_id)["status"] == "rejected"
    assert ctx.get_artifact(artifact_id)["review"]["status"] == "failed"
