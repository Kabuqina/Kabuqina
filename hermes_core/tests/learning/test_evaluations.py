...
def test_activate_and_reject_evaluations(ctx):
    ...
    assert [row["artifact_id"] for row in service.list_evaluations(status="active")] == [active_id]
...
def test_evaluation_is_owner_scoped(ctx):
    artifact_id = _draft(ctx)
    other = LearningExecutionContext(ctx._store, owner_id="owner-B", space_id="s1")
    with pytest.raises(KeyError):
        EvaluationService(other).activate_evaluation(artifact_id)
