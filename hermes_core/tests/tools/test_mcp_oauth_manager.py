...
def test_kabuqina_provider_subclass_exists():
    """KabuqinaMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _KABUQINA_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _KABUQINA_PROVIDER_CLS is not None
    assert issubclass(_KABUQINA_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_kabuqina_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns KabuqinaMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _KABUQINA_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("KABUQINA_HOME", str(tmp_path))
    ...
    assert _KABUQINA_PROVIDER_CLS is not None
    assert isinstance(provider, _KABUQINA_PROVIDER_CLS)
    assert provider._kabuqina_server_name == "srv"
