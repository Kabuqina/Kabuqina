...
``kabuqina_cli.config.get_env_value``) and not only from ``os.environ`` —
...
already addressed for ``agent/credential_pool`` and ``kabuqina_cli/auth``.
"""
...
class TestDotenvFallbackPerProvider:
    ...
    ``kabuqina_cli.config.load_env`` integration. Before the fix, ``os.getenv``
...
class TestRegressionGuard:
    ...
    ``kabuqina_cli.config.load_env`` to simulate ``~/.hermes/.env`` carrying the
    ...
    def test_import_after_config_env_patch_uses_restored_dotenv_loader(self, tmp_path, monkeypatch):
        """Importing TTS while kabuqina_cli.config.get_env_value is patched must
        not freeze that temporary helper into this module forever.
        """
        import importlib
        import kabuqina_cli.config as config_mod
        from tools import tts_tool
    ...
    def test_minimax_missing_when_only_in_dotenv_before_fix(self, tmp_path, monkeypatch):
        ...
        with patch(
            ...
            # os.environ is empty.
            from kabuqina_cli.config import get_env_value as live_get
