...
@pytest.fixture
def isolated_kabuqina_home(tmp_path, monkeypatch):
    """Point KABUQINA_HOME at a temp dir and clear known API key env vars.
...
class TestCredentialPoolSeedsFromDotEnv:
    ...
    load_kabuqina_dotenv, the credential pool must still discover it.
    """

    def test_deepseek_key_from_dotenv_only(self, isolated_kabuqina_home):
        """Key in .env but not os.environ → _seed_from_env adds a pool entry."""
        _write_env_file(isolated_kabuqina_home, DEEPSEEK_API_KEY="sk-dotenv-only-12345")
        assert "DEEPSEEK_API_KEY" not in os.environ

        from providers.credential_pool import _seed_from_env
        entries = []
    ...
    def test_openrouter_key_from_dotenv_only(self, isolated_kabuqina_home):
        """OpenRouter path has its own branch — verify it also reads .env."""
        _write_env_file(isolated_kabuqina_home, OPENROUTER_API_KEY="sk-or-dotenv-abc")
    ...
    def test_empty_dotenv_no_entries(self, isolated_kabuqina_home):
    ...
    def test_os_environ_still_wins_over_dotenv(self, isolated_kabuqina_home, monkeypatch):
        """get_env_value checks os.environ first — verify seeding picks that up."""
        _write_env_file(isolated_kabuqina_home, DEEPSEEK_API_KEY="sk-dotenv-stale")
...
class TestAuthResolvesFromDotEnv:
    ...
    def test_key_from_dotenv_only(self, isolated_kabuqina_home):
        """Key in .env but not os.environ → _resolve returns it with the env var source."""
        _write_env_file(isolated_kabuqina_home, DEEPSEEK_API_KEY="sk-dotenv-resolve-789")
        assert "DEEPSEEK_API_KEY" not in os.environ

        from kabuqina_cli.auth import _resolve_api_key_provider_secret
...
class TestAuthCredentialPoolFallback:
    ...
    def test_credential_pool_fallback_structure(self, isolated_kabuqina_home):
    ...
    def test_credential_pool_empty_returns_empty(self, isolated_kabuqina_home):
    ...
    def test_env_var_takes_priority_over_pool(self, isolated_kabuqina_home, monkeypatch):
        ...
        from kabuqina_cli.auth import _resolve_api_key_provider_secret
        with patch("providers.credential_pool.load_pool", return_value=mock_pool) as mp:
            key, source = _resolve_api_key_provider_secret(
                provider_id="deepseek",
                pconfig=_make_pconfig(),
            )
    ...
    def test_dotenv_takes_priority_over_pool(self, isolated_kabuqina_home):
        """Key in .env beats credential pool — pool only fires when both env sources are empty."""
        _write_env_file(isolated_kabuqina_home, DEEPSEEK_API_KEY="sk-dotenv-priority-xyz")
        assert "DEEPSEEK_API_KEY" not in os.environ
