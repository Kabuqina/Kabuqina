...
class TestHermesToolsGeneration(unittest.TestCase):
    ...
    def test_rpc_infrastructure_present(self):
        ...
        self.assertIn("KABUQINA_RPC_SOCKET", src)
    ...
    def test_file_transport_uses_tempfile_fallback_for_rpc_dir(self):
        ...
        self.assertIn("os.path.join(tempfile.gettempdir(), \"kabuqina_rpc\")", src)
        self.assertIn('os.environ.get("KABUQINA_RPC_DIR")', src)
        self.assertIn('os.environ.get("HERMES_RPC_DIR")', src)
...
class TestExecuteCodeRemoteTempDir(unittest.TestCase):
    def test_execute_remote_uses_backend_temp_dir_for_sandbox(self):
        ...
        self.assertIn("mkdir -p /data/data/com.termux/files/usr/tmp/kabuqina_exec_", mkdir_cmd)
        self.assertIn("KABUQINA_RPC_DIR=/data/data/com.termux/files/usr/tmp/kabuqina_exec_", run_cmd)
        self.assertIn("rm -rf /data/data/com.termux/files/usr/tmp/kabuqina_exec_", cleanup_cmd)
        self.assertNotIn("mkdir -p /tmp/kabuqina_exec_", mkdir_cmd)
...
class TestExecuteCode(unittest.TestCase):
    ...
    def test_repo_root_modules_are_importable(self):
        ...
        result = self._run('import kabuqina_constants; print(kabuqina_constants.__file__)')
        self.assertEqual(result["status"], "success")
        self.assertIn("kabuqina_constants.py", result["output"])
...
class TestBuildExecuteCodeSchema(unittest.TestCase):
    ...
    def test_real_scenario_all_sandbox_tools_disabled(self):
        ...
        Scenario: user runs `kabuqina tools code_execution` (only code_execution
    ...
    def test_real_scenario_only_vision_enabled(self):
        """Another real path: user runs `kabuqina tools code_execution,vision`.
...
class TestEnvVarFiltering(unittest.TestCase):
    ...
    def test_kabuqina_rpc_socket_injected(self):
        child_env = self._get_child_env()
        self.assertIn("KABUQINA_RPC_SOCKET", child_env)
...
class TestLoadConfig(unittest.TestCase):
    ...
    def test_returns_code_execution_section(self):
        from tools.code_execution_tool import _load_config
        with patch("kabuqina_cli.config.read_raw_config",
    ...
    def test_does_not_import_interactive_cli(self):
        ...
        with patch.dict("sys.modules", {"cli": mock_cli}), \
             patch("kabuqina_cli.config.read_raw_config", return_value={}):
            result = _load_config()
