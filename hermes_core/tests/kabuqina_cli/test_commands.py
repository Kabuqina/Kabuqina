...
class TestCommandRegistry:
    ...
    def test_every_entry_has_valid_category(self):
        valid_categories = {"Session", "Configuration", "Tools & Skills", "Study", "Info", "Exit"}
        for cmd in COMMAND_REGISTRY:
...
class TestSlackNativeSlashes:
    ...
    def test_kabuqina_catchall_is_first_and_legacy_alias_is_retained(self):
        """The canonical catch-all leads; the old slash remains available."""
        slashes = slack_native_slashes()
        assert slashes[0][0] == "kabuqina"
        assert slashes[1][0] == "hermes"
...
from kabuqina_cli.commands import discord_skill_commands_by_category  # noqa: E402
...
class TestPluginCommandEnumeration:
    ...
    def _patch_plugin_commands(self, monkeypatch, commands):
        """Monkeypatch kabuqina_cli.plugins.get_plugin_commands() to a fixed dict."""
        from kabuqina_cli import plugins as _plugins_mod
    ...
    def test_is_gateway_known_command_recognizes_plugin_commands(self, monkeypatch):
        ...
        from kabuqina_cli.commands import is_gateway_known_command
