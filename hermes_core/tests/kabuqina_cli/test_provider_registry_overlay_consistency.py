...
`KABUQINA_OVERLAYS` (`kabuqina_cli.providers`) and `PROVIDER_REGISTRY`
(`kabuqina_cli.auth`) are intentionally separate — they serve two subsystems:
...
* `KABUQINA_OVERLAYS` is the identity/routing overlay on the models.dev catalog —
...
def _aligned_overlay_ids():
    return [oid for oid in KABUQINA_OVERLAYS if _registry_for(oid) is not None]
...
def test_membership_asymmetry_is_as_declared():
    ...
    overlay_only = {oid for oid in KABUQINA_OVERLAYS if _registry_for(oid) is None}
