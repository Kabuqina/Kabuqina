...
def test_estimate_tool_tokens_returns_positive_counts():
    ...
    from kabuqina_cli.tools_config import _estimate_tool_tokens, _tool_token_cache

    # Clear cache to force fresh computation
    import kabuqina_cli.tools_config as tc
...
def test_prompt_toolset_checklist_passes_status_fn(monkeypatch):
    ...
    monkeypatch.setattr("kabuqina_cli.curses_ui.curses_checklist", fake_checklist)
...
def test_curses_checklist_numbered_fallback_shows_status(monkeypatch, capsys):
    ...
    from kabuqina_cli.curses_ui import _numbered_fallback
