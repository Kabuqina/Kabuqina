"""Tests for the Hermes plugin system (kabuqina_cli.plugins)."""
...
from kabuqina_cli.plugins import (
    ENTRY_POINTS_GROUP,
    VALID_HOOKS,
    LoadedPlugin,
    PluginContext,
...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    *base* is expected to be ``<hermes_home>/plugins/``; we derive
    ...
    if auto_enable:
        # Write/merge plugins.enabled in <KABUQINA_HOME>/config.yaml.
        # Config is always read from KABUQINA_HOME (not from the project
        # dir for project plugins), so that's where we opt in.
        import os
        hermes_home_str = os.environ.get("KABUQINA_HOME")
        if hermes_home_str:
...
class TestPluginDiscovery:
    ...
    def test_discover_user_plugins(self, tmp_path, monkeypatch):
        ...
        plugins_dir = tmp_path / "kabuqina_test" / "plugins"
        _make_plugin_dir(plugins_dir, "hello_plugin")
        monkeypatch.setenv("KABUQINA_HOME", str(tmp_path / "kabuqina_test"))
...
class TestPluginLoading:
    ...
    def test_load_missing_init(self, tmp_path, monkeypatch):
        ...
        # missing-init error.
        hermes_home = tmp_path / "kabuqina_test"
...
class TestPluginHooks:
    ...
    def test_invalid_hook_name_warns(self, tmp_path, monkeypatch, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="kabuqina_cli.plugins"):
            mgr = PluginManager()
            mgr.discover_and_load()
...
class TestPreToolCallBlocking:
    ...
    def test_block_message_returned_for_valid_directive(self, monkeypatch):
        monkeypatch.setattr(
            "kabuqina_cli.plugins.invoke_hook",
...
class TestPluginToolVisibility:
    ...
    def test_plugin_tools_in_definitions(self, tmp_path, monkeypatch):
        ...
        import kabuqina_cli.plugins as plugins_mod
...
class TestPluginCommands:
    ...
    def test_get_plugin_command_handler_found(self):
        ...
        with patch("kabuqina_cli.plugins._plugin_manager", mgr):
    ...
    def test_commands_in_list_plugins_output(self, tmp_path, monkeypatch):
        ...
        plugins_dir = tmp_path / "kabuqina_test" / "plugins"
        # Set KABUQINA_HOME BEFORE _make_plugin_dir so auto-enable targets
...
class TestPluginDispatchTool:
    ...
    def test_dispatch_tool_calls_registry(self):
        ...
        with patch("kabuqina_cli.plugins.PluginContext.dispatch_tool.__module__", "kabuqina_cli.plugins"):
