"""Tests for kabuqina_cli configuration management."""
...
class TestGetKabuqinaHome:
    def test_default_path(self, tmp_path):
        with patch.dict(os.environ, {}, clear=True), patch.object(Path, "home", return_value=tmp_path):
            home = get_kabuqina_home()
            assert home == tmp_path / ".kabuqina"

    def test_env_override(self):
        with patch.dict(os.environ, {"KABUQINA_HOME": "/custom/path"}, clear=True):
            home = get_kabuqina_home()
            assert home == Path("/custom/path")

    def test_legacy_env_fallback(self):
        with patch.dict(os.environ, {"HERMES_HOME": "/legacy/path"}, clear=True):
            home = get_kabuqina_home()
            assert home == Path("/legacy/path")


class TestEnsureKabuqinaHome:
    def test_legacy_home_helpers_remain_aliases(self):
        import hermes_cli.config_home as legacy
        import kabuqina_cli.config_home as canonical

        assert legacy.ensure_hermes_home is canonical.ensure_kabuqina_home
        assert (
            legacy._ensure_hermes_home_managed
            is canonical._ensure_kabuqina_home_managed
        )
...
class TestLoadConfigDefaults:
    ...
    def test_default_config_seeds_high_reasoning_for_first_run(self):
...
class TestSaveEnvValueSecure:
    ...
    def test_save_env_value_updates_process_environment(self, tmp_path):
        with patch.dict(os.environ, {"KABUQINA_HOME": str(tmp_path)}, clear=False):
...
class TestRemoveEnvValue:
    def test_removes_key_from_env_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"KABUQINA_HOME": str(tmp_path), "KEY_B": "value_b"}):
    ...
    def test_clears_os_environ(self, tmp_path):
        ...
        with patch.dict(os.environ, {"KABUQINA_HOME": str(tmp_path), "MY_KEY": "my_value"}):
    ...
    def test_handles_missing_env_file(self, tmp_path):
        with patch.dict(os.environ, {"KABUQINA_HOME": str(tmp_path), "GHOST_KEY": "ghost"}):
    ...
    def test_clears_os_environ_even_when_not_in_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"KABUQINA_HOME": str(tmp_path), "ORPHAN_KEY": "orphan"}):
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_tavily_api_key_registered(self):
        ...
        from kabuqina_cli.config import OPTIONAL_ENV_VARS
    ...
    def test_tavily_in_env_vars_by_version(self):
        ...
        from kabuqina_cli.config import ENV_VARS_BY_VERSION
        all_vars = []
