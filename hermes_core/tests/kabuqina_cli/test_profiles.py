"""Comprehensive tests for kabuqina_cli.profiles module.
...
from kabuqina_cli.profiles import (
    ...
    _get_profiles_root,
    _get_default_kabuqina_home,
)


# ---------------------------------------------------------------------------
# Shared fixture: redirect Path.home() and KABUQINA_HOME for profile tests
# ---------------------------------------------------------------------------
...
def profile_env(tmp_path, monkeypatch):
    ...
    * KABUQINA_HOME  -> tmp_path/.hermes  (so get_kabuqina_home() agrees)
    * Creates the bare-minimum ~/.hermes directory.
    """
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    default_home = tmp_path / ".hermes"
    default_home.mkdir(exist_ok=True)
    monkeypatch.setenv("KABUQINA_HOME", str(default_home))
    return tmp_path
...
class TestGetProfileDir:
    ...
    def test_default_returns_kabuqina_home(self, profile_env):
...
class TestDeleteProfile:
    ...
    def test_removes_directory(self, profile_env):
        ...
        with patch("kabuqina_cli.profiles._stop_gateway_process"):
...
class TestGetActiveProfileName:
    ...
    def test_default_kabuqina_home_returns_default(self, profile_env):
        # KABUQINA_HOME points to tmp_path/.hermes which is the default
...
class TestRenameProfile:
    ...
    def test_renames_directory(self, profile_env):
        ...
        with patch("kabuqina_cli.profiles.check_alias_collision", return_value="skip"):
...
class TestCompletion:
    ...
    def test_bash_completion_has_kabuqina_profiles_function(self):
        script = generate_bash_completion()
        assert "_kabuqina_profiles" in script

    def test_zsh_completion_has_kabuqina_function(self):
        script = generate_zsh_completion()
        assert "_kabuqina" in script
...
class TestInternalHelpers:
    """Tests for _get_profiles_root() and _get_default_kabuqina_home()."""
    ...
    def test_default_kabuqina_home(self, profile_env):
        tmp_path = profile_env
        home = _get_default_kabuqina_home()
        assert home == tmp_path / ".hermes"

    def test_profiles_root_docker_deployment(self, tmp_path, monkeypatch):
        """In Docker (KABUQINA_HOME outside ~/.hermes), profiles go under KABUQINA_HOME."""
    ...
    def test_default_kabuqina_home_docker(self, tmp_path, monkeypatch):
        """In Docker, _get_default_kabuqina_home() returns KABUQINA_HOME itself."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("KABUQINA_HOME", str(docker_home))
    ...
    def test_profiles_root_profile_mode(self, tmp_path, monkeypatch):
        """In profile mode (KABUQINA_HOME under ~/.hermes), profiles root is still ~/.hermes/profiles."""
    ...
    def test_active_profile_path_docker(self, tmp_path, monkeypatch):
        """In Docker, active_profile file lives under KABUQINA_HOME."""
        from kabuqina_cli.profiles import _get_active_profile_path
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("KABUQINA_HOME", str(docker_home))
    ...
    def test_create_profile_docker(self, tmp_path, monkeypatch):
        """Profile created in Docker lands under KABUQINA_HOME/profiles/."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("KABUQINA_HOME", str(docker_home))
...
class TestEdgeCases:
    ...
    def test_gateway_running_check_with_pid_file(self, profile_env):
        ...
        from kabuqina_cli.profiles import _check_gateway_running
        tmp_path = profile_env
        default_home = tmp_path / ".hermes"
