"""Tests for the `kabuqina memory reset` CLI command.
...
- Profile-scoped reset (uses KABUQINA_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake KABUQINA_HOME with memory files."""
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `kabuqina memory reset` is run.
    """
    from kabuqina_constants import get_kabuqina_home, display_kabuqina_home

    mem_dir = get_kabuqina_home() / "memories"
    files_to_reset = []
...
class TestMemoryReset:
    """Tests for `kabuqina memory reset` subcommand."""
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's KABUQINA_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        # The memories dir won't exist; get_kabuqina_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"
