...
from kabuqina_cli.auth import AuthError, get_provider_auth_state, resolve_nous_runtime_credentials
...
class TestResolveVerifyFallback:
    ...
    def test_missing_ca_bundle_in_auth_state_falls_back(self):
        from kabuqina_cli.auth import _resolve_verify
...
def test_get_nous_auth_status_checks_credential_pool(tmp_path, monkeypatch):
    ...
    """
    from kabuqina_cli.auth import get_nous_auth_status
    ...
    from providers.credential_pool import PooledCredential, load_pool
    pool = load_pool("nous")
...
def test_get_nous_auth_status_auth_store_fallback(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("KABUQINA_HOME", str(hermes_home))
    monkeypatch.setattr(
        "providers.nous_auth.resolve_nous_runtime_credentials",
...
def test_get_nous_auth_status_reports_revoked_refresh_session(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("providers.nous_auth.resolve_nous_runtime_credentials", _boom)
...
def test_refresh_token_persisted_when_mint_returns_insufficient_credits(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("providers.nous_auth._refresh_access_token", _fake_refresh_access_token)
    monkeypatch.setattr("providers.nous_auth._mint_agent_key", _fake_mint_agent_key)
...
class TestLoginNousSkipKeepsCurrent:
    """When a user runs `kabuqina model` → Nous Portal → Skip (keep current) after
    ...
    def _patch_login_internals(self, monkeypatch, *, prompt_returns):
        ...
        import kabuqina_cli.auth as auth_mod
        import kabuqina_cli.models as models_mod
        import kabuqina_cli.nous_subscription as ns
    ...
    def test_skip_keep_current_preserves_provider_and_model(self, tmp_path, monkeypatch):
        ...
        import argparse
        import yaml
        from kabuqina_cli.auth import PROVIDER_REGISTRY, _login_nous
...
def test_persist_nous_credentials_writes_both_pool_and_providers(tmp_path, monkeypatch):
    ...
    Regression guard: before this helper existed, `kabuqina auth add nous`
    ...
    """
    from kabuqina_cli.auth import persist_nous_credentials, NOUS_DEVICE_CODE_SOURCE
...
def test_persist_nous_credentials_allows_recovery_from_401(tmp_path, monkeypatch):
    ...
    """
    from kabuqina_cli.auth import persist_nous_credentials, resolve_nous_runtime_credentials
...
def test_persist_nous_credentials_embeds_custom_label(tmp_path, monkeypatch):
    ...
    Previously `kabuqina auth add nous --type oauth --label <name>` silently
...
def test_persist_nous_credentials_custom_label_survives_reseed(tmp_path, monkeypatch):
    ...
    """
    from kabuqina_cli.auth import persist_nous_credentials
...
def test_refresh_token_reuse_detection_surfaces_actionable_message():
    ...
    """
    from kabuqina_cli.auth import _refresh_access_token
    ...
    assert "kabuqina auth add nous" in message.lower()
