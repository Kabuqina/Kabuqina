...
The terminal `kabuqina model` flow already exposes `custom_providers`, but the
shared slash-command pipeline (`/model` in CLI/gateway/Telegram) historically
only looked at `providers:`.
"""

import kabuqina_cli.providers as providers_mod
from kabuqina_cli.model_switch import list_authenticated_providers, switch_model
from kabuqina_cli.providers import resolve_provider_full
...
def test_list_groups_same_name_custom_providers_into_one_row(monkeypatch):
    ...
    monkeypatch.setattr(providers_mod, "KABUQINA_OVERLAYS", {})

    providers = list_authenticated_providers(
        current_provider="openrouter",
        user_providers={},
        custom_providers=[
            {"name": "Remote Gateway", "base_url": "https://api.example.com/v1", "model": "qwen3-coder:480b-cloud"},
            {"name": "Remote Gateway", "base_url": "https://api.example.com/v1", "model": "glm-5.1:cloud"},
            {"name": "Remote Gateway", "base_url": "https://api.example.com/v1", "model": "kimi-k2.5"},
            {"name": "Remote Gateway", "base_url": "https://api.example.com/v1", "model": "minimax-m2.7:cloud"},
            {"name": "Moonshot", "base_url": "https://api.moonshot.ai/v1", "model": "kimi-k2-thinking"},
        ],
        max_models=50,
    )

    gateway_rows = [p for p in providers if p["name"] == "Remote Gateway"]
    assert len(gateway_rows) == 1, f"Expected 1 Remote Gateway row, got {len(gateway_rows)}"
    assert gateway_rows[0]["models"] == [
        "qwen3-coder:480b-cloud", "glm-5.1:cloud", "kimi-k2.5", "minimax-m2.7:cloud"
    ]
    assert gateway_rows[0]["total_models"] == 4
...
def test_lmstudio_picker_probes_active_config_base_url(monkeypatch):
    ...
    monkeypatch.setattr("kabuqina_cli.models.fetch_lmstudio_models", _fake_fetch)
