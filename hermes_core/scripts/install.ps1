...
function Test-Node {
    ...
    # Fallback: download binary zip to ~/.kabuqina/node/
    ...
    try {
        ...
        if ($zipName) {
            ...
            if ($extractedDir) {
                ...
                $version = & "$HermesHome\node\node.exe" --version
                Write-Success "Node.js $version installed to ~/.kabuqina/node/"
                $script:HasNode = $true
...
function Copy-ConfigTemplates {
    ...
    # Create ~/.kabuqina directory structure
    ...
    if (-not (Test-Path $envPath)) {
        ...
        if (Test-Path $examplePath) {
            ...
            Write-Success "Created ~/.kabuqina/.env from template"
        } else {
            New-Item -ItemType File -Force -Path $envPath | Out-Null
            Write-Success "Created ~/.kabuqina/.env"
        }
    } else {
        Write-Info "~/.kabuqina/.env already exists, keeping it"
    }
    ...
    if (-not (Test-Path $configPath)) {
        ...
        if (Test-Path $examplePath) {
            ...
            Write-Success "Created ~/.kabuqina/config.yaml from template"
        }
    } else {
        Write-Info "~/.kabuqina/config.yaml already exists, keeping it"
    }
...
"@ | Set-Content -Path $soulPath -Encoding UTF8
        Write-Success "Created ~/.kabuqina/SOUL.md (edit to customize personality)"
    }
    
    Write-Success "Configuration directory ready: ~/.kabuqina/"
    
    # Seed bundled skills into ~/.kabuqina/skills/ (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to ~/.kabuqina/skills/ ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to ~/.kabuqina/skills/"
        } catch {
            ...
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                ...
                Write-Success "Skills copied to ~/.kabuqina/skills/"
            }
        }
    }
}
...
function Start-GatewayIfConfigured {
    ...
    foreach ($var in @("TELEGRAM_BOT_TOKEN", "SLACK_BOT_TOKEN", "SLACK_APP_TOKEN", "WHATSAPP_ENABLED")) {
...
function Write-Completion {
    ...
    Write-Host "Start messaging gateway (Telegram, Slack, WhatsApp, etc.)"
