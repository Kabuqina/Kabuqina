...
class StudentStateService:
    ...
    def activate_state(self, artifact_id: str) -> Dict[str, Any]:
        self._require_state(artifact_id)
        self._ctx.activate_singleton_artifact(artifact_id, kind="student_state")
        return self._public(self._require_state(artifact_id))
    ...
    def normalize_payload(payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "dimensions" in payload:
            normalized["dimensions"] = _dimensions(payload.get("dimensions"))
        return normalized
    ...
    def legacy_context_to_payloads(
        ...
        state = {
            ...
            "constraints": [],
            "progress_notes": _merge_strings(
                context.get("progressNotes"), context.get("generatedResources")
            ),
            "current_stage": _clean_text(context.get("currentStage")),
            "next_adjustment": _clean_text(context.get("nextAdjustment")),
        }
        observations = _merge_strings(
            context.get("evaluationSummary"),
            context.get("assessmentEvidence"),
            context.get("tutoringNotes"),
        )
