...
"""Config file/dir path resolvers for hermes_cli.

Extracted from ``hermes_cli/config.py`` as a clean leaf (only ``get_hermes_home``
from ``hermes_constants``): the ``config.yaml`` path, the install/project root,
and the atomic dotted-key writer. Sits below the loader core; ``hermes_cli.config``
