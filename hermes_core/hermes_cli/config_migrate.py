...
"""Config-version migration for hermes_cli (the ``migrate_config`` flow).

Extracted from ``hermes_cli/config.py``: a one-shot, interactive migration that
...
to avoid an import cycle. ``hermes_cli.config`` re-exports ``migrate_config``.
"""
...
from hermes_cli.config_env_schema import ENV_VARS_BY_VERSION, OPTIONAL_ENV_VARS
from hermes_cli.config_env import get_env_value, save_env_value, sanitize_env_file
from hermes_cli.config_loader import load_config, read_raw_config, save_config
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    """
    # Inspection helpers live in the hermes_cli.config facade (shared with the
