...
"""Environment-variable schema data for hermes_cli.

Extracted verbatim from ``hermes_cli/config.py`` to shrink that module: the
per-version env-var map and the required/optional env-var metadata used by the
setup checklists and migration. Pure data, no logic; ``hermes_cli.config``
