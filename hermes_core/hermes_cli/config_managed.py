...
"""Managed-install and container detection for hermes_cli.

Extracted from ``hermes_cli/config.py`` as a self-contained leaf (depends only
on ``get_hermes_home`` from ``hermes_constants``): whether this install is
...
from here. ``hermes_cli.config`` re-exports every name so existing imports work.
"""
