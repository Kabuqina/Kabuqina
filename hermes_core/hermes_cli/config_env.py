...
"""Read/write helpers for the user ``~/.hermes/.env`` file.

Extracted from ``hermes_cli/config.py``: env-file load/save/sanitize, secret
...
(ensure_hermes_home/_secure_file); reads the schema from ``config_env_schema``.
``hermes_cli.config`` re-exports every name so existing imports and test
...
from hermes_cli.config_managed import is_managed, managed_error
from hermes_cli.config_home import ensure_hermes_home, _secure_file
from hermes_cli.config_env_schema import OPTIONAL_ENV_VARS
