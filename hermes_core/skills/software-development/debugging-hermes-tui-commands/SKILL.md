...
```
Python backend (kabuqina_cli/commands.py)     <- canonical COMMAND_REGISTRY
       │
       ▼
...
3. **Check if the command exists in the Python backend:**
   ```bash
   search_files --pattern "CommandDef" --file_glob "*.py" --path kabuqina_cli/
   search_files --pattern "commandname" --path kabuqina_cli/commands.py --context 3
   ```
...
1. **Command shows in TUI but not in autocomplete.** The command is defined in the TUI codebase but missing from `COMMAND_REGISTRY` in `kabuqina_cli/commands.py`. Autocomplete data ships from Python.
...
4. Execute the command and confirm:
   ...
   - Any persisted config updates correctly (`read_file ~/.kabuqina/config.yaml`)
