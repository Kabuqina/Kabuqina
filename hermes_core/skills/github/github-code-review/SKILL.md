...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if [ -f ~/.kabuqina/.env ] && grep -q "^GITHUB_TOKEN=" ~/.kabuqina/.env; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.kabuqina/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
...
```bash
source "${KABUQINA_HOME:-$HOME/.kabuqina}/skills/github/github-auth/scripts/gh-env.sh"
