...
from _kabuqina_home import display_kabuqina_home, get_kabuqina_home

KABUQINA_HOME = get_kabuqina_home()
TOKEN_PATH = KABUQINA_HOME / "google_token.json"
CLIENT_SECRET_PATH = KABUQINA_HOME / "google_client_secret.json"
PENDING_AUTH_PATH = KABUQINA_HOME / "google_oauth_pending.json"
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same Kabuqina profile to refresh consent."
    )
...
def install_deps():
    ...
    try:
        ...
        print("  pip install 'kabuqina-agent[google]'")
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_kabuqina_home()}/google_token.json")
