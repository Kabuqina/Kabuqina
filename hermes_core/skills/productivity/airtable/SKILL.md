...
4. Store the token in `~/.kabuqina/.env` (or via `kabuqina setup`):
   ```
...
- **`AIRTABLE_API_KEY` flows from `~/.kabuqina/.env` into the subprocess automatically** when this skill is loaded — no need to re-export it before each `curl` call.
