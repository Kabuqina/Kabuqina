...
Data resolution order (like TypeScript OpenCode):
  ...
  2. Disk cache (~/.kabuqina/models_dev_cache.json)
...
# ---------------------------------------------------------------------------
# Provider ID mapping: Kabuqina ↔ models.dev
# ---------------------------------------------------------------------------

# Kabuqina provider names → models.dev provider IDs
...
# Reverse mapping: models.dev → Kabuqina (built lazily)
...
def _get_cache_path() -> Path:
    ...
    return get_kabuqina_home() / "models_dev_cache.json"
...
def _get_provider_models(provider: str) -> Optional[Dict[str, Any]]:
    """Resolve a Kabuqina provider ID to its models dict from models.dev.
...
def list_provider_models(provider: str) -> List[str]:
    ...
    """
    from kabuqina_cli.models import normalize_provider
...
# Gemma models whose TPM quotas are too small for normal Kabuqina agent traffic.
...
def get_provider_info(provider_id: str) -> Optional[ProviderInfo]:
    ...
    Accepts either a Kabuqina provider ID (e.g. "openrouter") or a models.dev
    ID.  Returns None if the provider is not in the catalog.
    """
    # Resolve Kabuqina ID → models.dev ID
...
def get_model_info(
    ...
    Accepts Kabuqina or models.dev provider ID.  Tries exact match then
