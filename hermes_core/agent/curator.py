...
def _state_file() -> Path:
    return get_kabuqina_home() / "skills" / ".curator_state"
...
def _load_config() -> Dict[str, Any]:
    """Read curator.* config from ~/.kabuqina/config.yaml. Tolerates missing file."""
    try:
...
CURATOR_REVIEW_PROMPT = (
    "You are running as Kabuqina' background skill CURATOR. This is an "
    ...
    "into ~/.kabuqina/skills/.archive/) is the maximum destructive action. "
    ...
    "to find: kabuqina-config-*, kabuqina-dashboard-*, gateway-*, cli-*, "
    ...
    "~/.kabuqina/skills/<umbrella>/references/ && mv ... <umbrella>/"
...
def _reports_root() -> Path:
    ...
    Lives under the profile-aware logs dir (``~/.kabuqina/logs/curator/``)
    ...
    authored skill data in ``~/.kabuqina/skills/``.

    ``ensure_kabuqina_home()`` pre-creates this dir on every CLI launch and
    ...
    that bypasses both.
    """
    root = get_kabuqina_home() / "logs" / "curator"
    try:
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    if archived:
        ...
        lines.append("_Archived skills are at `~/.kabuqina/skills/.archive/`. "
                     "Restore any via `kabuqina curator restore <name>`._\n")
    ...
    lines.append("- Restore an archived skill: `kabuqina curator restore <name>`")
    lines.append("- All archives live under `~/.kabuqina/skills/.archive/` and are recoverable by `mv`")
...
def run_curator_review(
    ...
    def _llm_pass():
        ...
        # recorded in state so `kabuqina curator status` can point at it.
        try:
...
def _resolve_review_model(cfg: Dict[str, Any]) -> tuple[str, str]:
    ...
    — so it participates in the canonical aux-model plumbing (``kabuqina model`` →
...
def _run_llm_review(prompt: str) -> Dict[str, Any]:
    ...
    # (canonical aux-task slot, wired through `kabuqina model` → auxiliary
