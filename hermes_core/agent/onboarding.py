...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Kabuqina starts and finds ``~/.openclaw/``.

    Points users at ``kabuqina claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``kabuqina claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Kabuqina, run "
        "`kabuqina claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `kabuqina claw cleanup` (renames it to ~/.openclaw.pre-migration — "
