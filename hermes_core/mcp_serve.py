"""
Kabuqina MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Kabuqina-specific extra)

Usage:
    kabuqina mcp serve
    kabuqina mcp serve --verbose
...
logger = logging.getLogger("kabuqina.mcp_serve")
...
def _get_home() -> Path:
    """Return the canonical Kabuqina home with one-release fallback."""
    try:
        from kabuqina_constants import get_kabuqina_home
        return get_kabuqina_home()
    except ImportError:
        if "KABUQINA_HOME" in os.environ:
            return Path(os.environ["KABUQINA_HOME"] or Path.home() / ".kabuqina")
        return Path(os.environ.get("HERMES_HOME", Path.home() / ".kabuqina"))


def _get_sessions_dir() -> Path:
    """Return the sessions directory under KABUQINA_HOME."""
    return _get_home() / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    directory_file = _get_home() / "channel_directory.json"
...
class EventBridge:
    ...
    This is the Kabuqina equivalent of OpenClaw's WebSocket gateway bridge.
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Kabuqina MCP server with all tools registered."""
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Kabuqina MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
