"""``kabuqina logs`` — view and filter Kabuqina log files.
...
Usage examples::
    ...
    kabuqina logs                    # last 50 lines of agent.log
    kabuqina logs -f                 # follow agent.log in real time
    kabuqina logs errors             # last 50 lines of errors.log
    kabuqina logs gateway -n 100    # last 100 lines of gateway.log
    kabuqina logs --level WARNING    # only WARNING+ lines
    kabuqina logs --session abc123   # filter by session ID substring
    kabuqina logs --component tools  # only tool-related lines
    kabuqina logs --since 1h         # lines from the last hour
    kabuqina logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_kabuqina_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Kabuqina runs — try 'kabuqina chat' first)")
        sys.exit(1)
    ...
    if component:
        from kabuqina_logging import COMPONENT_PREFIXES
    ...
    if follow:
        print(f"--- {display_kabuqina_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_kabuqina_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_kabuqina_home()}/logs/")
        return

    print(f"Log files in {display_kabuqina_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'kabuqina chat' to generate logs)")
