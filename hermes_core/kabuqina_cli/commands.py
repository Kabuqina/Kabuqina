...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of Kabuqina config/state", "Session",
    ...
    CommandDef("deny", "Deny a pending dangerous command", "Session",
               gateway_only=True),
    CommandDef("study", "Manage personal study spaces and drafts", "Study",
               gateway_only=True, args_hint="[list|new|use|drafts|approve|reject]"),
    ...
    CommandDef("busy", "Control what Enter does while Kabuqina is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.kabuqina/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update Kabuqina to the latest version", "Info",
               gateway_only=True),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    """
    try:
        from kabuqina_cli.plugins import get_plugin_commands
    except Exception:
        return []
    try:
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``kabuqina skills
