"""
Single source of truth for provider identity in Kabuqina.
...
2. **Kabuqina overlays** — transport type, auth patterns, aggregator flags,
...
# -- Kabuqina overlay --------------------------------------------------------
# Kabuqina-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class KabuqinaOverlay:
    """Kabuqina-specific provider metadata layered on top of models.dev."""
...
KABUQINA_OVERLAYS: Dict[str, KabuqinaOverlay] = {
    "openrouter": KabuqinaOverlay(
        transport="openai_chat",
        is_aggregator=True,
        extra_env_vars=("OPENAI_API_KEY",),
        base_url_env_var="OPENROUTER_BASE_URL",
    ),
    "nous": KabuqinaOverlay(
        transport="openai_chat",
        auth_type="oauth_device_code",
        base_url_override="https://inference-api.nousresearch.com/v1",
    ),
    "lmstudio": KabuqinaOverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "kimi-for-coding": KabuqinaOverlay(
        transport="openai_chat",
        base_url_env_var="KIMI_BASE_URL",
    ),
    "stepfun": KabuqinaOverlay(
        transport="openai_chat",
        extra_env_vars=("STEPFUN_API_KEY",),
        base_url_override="https://api.stepfun.ai/step_plan/v1",
        base_url_env_var="STEPFUN_BASE_URL",
    ),
    "minimax": KabuqinaOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_BASE_URL",
    ),
    "minimax-oauth": KabuqinaOverlay(
        transport="anthropic_messages",
        # Match PROVIDER_REGISTRY (kabuqina_cli/auth.py), which uses the
        ...
        # tests/kabuqina_cli/test_provider_registry_overlay_consistency.py.
        auth_type="oauth_minimax",
        base_url_override="https://api.minimax.io/anthropic",
    ),
    "minimax-cn": KabuqinaOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": KabuqinaOverlay(
        transport="openai_chat",
        base_url_env_var="DEEPSEEK_BASE_URL",
    ),
    "alibaba": KabuqinaOverlay(
        transport="openai_chat",
        base_url_env_var="DASHSCOPE_BASE_URL",
    ),
    "alibaba-coding-plan": KabuqinaOverlay(
        transport="openai_chat",
        base_url_env_var="ALIBABA_CODING_PLAN_BASE_URL",
    ),
    "huggingface": KabuqinaOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "xiaomi": KabuqinaOverlay(
        transport="openai_chat",
        base_url_env_var="XIAOMI_BASE_URL",
    ),
    "tencent-tokenhub": KabuqinaOverlay(
        transport="openai_chat",
        base_url_env_var="TOKENHUB_BASE_URL",
    ),
}

# Deprecated read-only aliases for one release. Internal code uses the
# canonical names; consumers that import the legacy names still see the same
# class and mapping objects.
HermesOverlay = KabuqinaOverlay
HERMES_OVERLAYS = KABUQINA_OVERLAYS
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "kabuqina", "user-config"
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Kabuqina overlays (for providers not in models.dev, such as nous)
      2. models.dev catalog + Kabuqina overlay
    ...
    overlay = KABUQINA_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + Kabuqina extra
    ...
    if overlay is not None:
        # Kabuqina-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
