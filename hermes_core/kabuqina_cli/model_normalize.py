...
- **DeepSeek** accepts ``deepseek-chat`` (V3), ``deepseek-reasoner``
  ...
  Kabuqina revisions folded every non-reasoner input into
...
def _normalize_provider_alias(provider_name: str) -> str:
    """Resolve provider aliases to Kabuqina' canonical ids."""
...
def normalize_model_for_provider(model_input: str, target_provider: str) -> str:
    ...
    Args:
        ...
        target_provider: The canonical Kabuqina provider id, e.g.
            ``"openrouter"``, ``"anthropic"``, ``"deepseek"``,
            ``"custom"``.  Should already be normalised
            via ``kabuqina_cli.models.normalize_provider()``.
