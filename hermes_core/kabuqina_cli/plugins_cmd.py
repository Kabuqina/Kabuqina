"""``kabuqina plugins`` CLI subcommand — install, update, remove, and list plugins.

Plugins are installed from Git repositories into ``~/.kabuqina/plugins/``.
...
def _plugins_dir() -> Path:
    ...
    plugins = get_kabuqina_home() / "plugins"
...
def _prompt_plugin_env_vars(manifest: dict, console) -> None:
    ...
    from kabuqina_cli.config import get_env_value, save_env_value  # noqa: F811
    ...
    for spec in missing:
        ...
        try:
            ...
            console.print(f"\n[dim]  Skipped (you can set these later in {display_kabuqina_home()}/.env)[/dim]")
            return

        if value:
            save_env_value(name, value)
            os.environ[name] = value
            console.print(f"  [green]✓[/green] Saved to {display_kabuqina_home()}/.env")
        else:
            console.print(f"  [dim]  Skipped (set {name} in {display_kabuqina_home()}/.env later)[/dim]")
...
def cmd_install(
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        if mv is not None:
            ...
            if mv_int > _SUPPORTED_MANIFEST_VERSION:
                from kabuqina_cli.config import recommended_update_command
                console.print(
        ...
        if target.exists():
            if not force:
                console.print(
                    ...
                    f"[bold]kabuqina plugins update {plugin_name}[/bold] to pull latest."
                )
                sys.exit(1)
    ...
    if not (target / "plugin.yaml").exists() and not (target / "__init__.py").exists():
        console.print(
            ...
            f"or __init__.py. It may not be a valid Kabuqina plugin."
        )
    ...
    if should_enable:
        ...
        console.print(
            ...
            f"Run `kabuqina plugins enable {installed_name}` to activate.[/dim]"
        )

    console.print("[dim]Restart the gateway for the plugin to take effect:[/dim]")
    console.print("[dim]  kabuqina gateway restart[/dim]")
    console.print()
...
def cmd_list() -> None:
    ...
    if not entries:
        ...
        console.print("[dim]Install with:[/dim] kabuqina plugins install owner/repo")
        return
    ...
    console.print(table)
    console.print()
    console.print("[dim]Interactive toggle:[/dim] kabuqina plugins")
    console.print("[dim]Enable/disable:[/dim] kabuqina plugins enable/disable <name>")
...
def plugins_command(args) -> None:
    """Dispatch kabuqina plugins subcommands."""
