...
"""Default configuration tree for kabuqina_cli.

Extracted verbatim from ``kabuqina_cli/config.py`` to shrink that module. Pure
data, no logic; ``kabuqina_cli.config`` re-exports ``DEFAULT_CONFIG`` so existing
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Kabuqina-level retry loop that wraps the whole call.  Lower
    ...
    "terminal": {
        ...
        # skipped. When empty, Kabuqina auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Kabuqina sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Kabuqina
        ...
        #  "/home/user/.kabuqina/cache/documents:/output"]
        ...
        # bundled Kabuqina image, which drops to the `hermes` user via gosu).
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Kabuqina sends a stable profile-scoped userId to Camofox
    ...
    "checkpoints": {
        ...
        # ~/.kabuqina/checkpoints/ (one per cd'd working directory). Field
        # reports put the typical offender at 1000+ repos / ~12 GB. When
        # auto_prune is on, Kabuqina sweeps at startup (at most once per
    ...
    # single read_file page exceeds these limits, Kabuqina truncates the
    ...
    # When ``provider: auto`` cannot satisfy a task, Kabuqina tries the auxiliary
    ...
    "auxiliary": {
        ...
        # model; override via `kabuqina model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # When true, `kabuqina --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `kabuqina -c` muscle memory. Default off so existing
    ...
    "tts": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.kabuqina/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.kabuqina/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # always goes to ~/.kabuqina/skills/.
    "skills": {
    ...
    # cron daemon.
    #
    # See `kabuqina curator status` for the last run summary.
    "curator": {
        "enabled": True,
    ...
    # This section is only needed for Kabuqina-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *Kabuqina*" header.
    ...
    # stored approval from ~/.kabuqina/shell-hooks-allowlist.json.
    ...
    "code_execution": {
        ...
        #     kabuqina-agent's own python (sys.executable). Maximum isolation
    ...
    # Logging — controls file logging to ~/.kabuqina/logs/.
    ...
    # update model picker lists without shipping a kabuqina-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://hermes-agent.nousresearch.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `kabuqina model` invocation; network failures
    ...
    # Session storage — controls automatic cleanup of ~/.kabuqina/state.db.
    ...
    "sessions": {
        ...
        # default of ``kabuqina sessions prune``.
        "retention_days": 90,
    ...
    # ``kabuqina update`` behaviour.
    "updates": {
        # Run a full ``kabuqina backup``-style zip of HERMES_HOME before every
        # ``kabuqina update``.  Backups land in ``<HERMES_HOME>/backups/`` and
        # can be restored with ``kabuqina import <path>``. Off by default —
