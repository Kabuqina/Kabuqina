...
- ``kabuqina_cli.providers``        -- canonical provider identity + overlays
- ``kabuqina_cli.model_normalize``  -- per-provider name formatting
...
from kabuqina_cli.providers import (
...
_HERMES_MODEL_WARNING = (
    ...
    "for use with Kabuqina. They lack the tool-calling capabilities "
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from kabuqina_cli.model_switch import
...
def resolve_alias(
    ...
    try:
        from kabuqina_cli.models import _PROVIDER_MODELS
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'kabuqina model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'kabuqina doctor' — config issues detected:"
        ...
        if not new_model:
            if pdef.base_url:
                from kabuqina_cli.runtime_provider import _auto_detect_local_model
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from kabuqina_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from kabuqina_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by provider ID
    ...
    if "lmstudio" not in curated and (
        ...
        from kabuqina_cli.models import fetch_lmstudio_models
    ...
    # --- 1. Check Kabuqina-mapped providers ---
    for provider_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        # section 2 (KABUQINA_OVERLAYS) with proper auth store checking.
        ...
        if not has_creds:
            try:
                from kabuqina_cli.auth import _load_auth_store
                store = _load_auth_store()
                if store and provider_id in store.get("credential_pool", {}):
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
        ...
        # show up in the picker without requiring a Kabuqina release.
        model_ids = curated.get(provider_id, [])
        if provider_id in _MODELS_DEV_PREFERRED:
            model_ids = _merge_with_models_dev(provider_id, model_ids)
        total = len(model_ids)
    ...
    # --- 2. Check Kabuqina-only providers (for example nous) ---
    from kabuqina_cli.providers import KABUQINA_OVERLAYS
    from kabuqina_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Kabuqina provider ID.
    # KABUQINA_OVERLAYS keys may be models.dev IDs while _PROVIDER_MODELS
    # and config.yaml use Kabuqina IDs.
    _mdev_to_provider = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in KABUQINA_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve Kabuqina slug.
        provider_slug = _mdev_to_provider.get(pid, pid)
        if provider_slug.lower() in seen_slugs:
            continue
        ...
        if not has_creds:
            try:
                ...
                if store and (
                    pid in providers_store or provider_slug in providers_store
                    or pid in pool_store or provider_slug in pool_store
                ):
                    has_creds = True
        ...
        if not has_creds:
            try:
                ...
                logger.debug("Credential pool check failed for %s: %s", provider_slug, exc)
        ...
        # configured.
        if not has_creds and provider_slug == "anthropic":
            try:
                from providers.anthropic import (
                    read_claude_code_credentials,
                    read_hermes_oauth_credentials,
                )
                oauth_creds = read_hermes_oauth_credentials()
                cc_creds = read_claude_code_credentials()
                if (oauth_creds and oauth_creds.get("accessToken")) or \
        ...
        # Use curated list — look up by Kabuqina slug, fall back to overlay key.
        model_ids = curated.get(provider_slug, []) or curated.get(pid, [])
        # Merge with models.dev for preferred providers (same rationale as above).
        if provider_slug in _MODELS_DEV_PREFERRED:
            model_ids = _merge_with_models_dev(provider_slug, model_ids)
        total = len(model_ids)
        top = model_ids[:max_models]
        ...
        results.append({
            ...
            "is_current": provider_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "kabuqina",
        ...
        seen_slugs.add(provider_slug.lower())
        _record_builtin_endpoint(provider_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or KABUQINA_OVERLAYS (keeps /model in sync
    # with `kabuqina model`).
    try:
        from kabuqina_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is Kabuqina's canonical write key (matches
            ...
            # Kabuqina writes ``models:`` as a dict keyed by model id
            # (see kabuqina_cli/main.py::_save_custom_provider); older
            ...
            if api_url and api_key:
                try:
                    from kabuqina_cli.models import fetch_api_models
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # Kabuqina's own writer uses; a hyphen variant is accepted
            ...
            # active model. Kabuqina's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, kabuqina_cli/config.py) already consume that dict.
