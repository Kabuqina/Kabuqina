...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or kabuqina -c.",
    ...
    "Ctrl+Z suspends Kabuqina to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "kabuqina -c resumes your most recent CLI session. kabuqina -c \"project name\" resumes by title.",
    "kabuqina -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "kabuqina -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "kabuqina chat -t web,terminal enables only specific toolsets for a focused session.",
    "kabuqina chat -s github-pr-workflow preloads a skill at launch.",
    "kabuqina chat -q \"query\" runs a single non-interactive query and exits.",
    "kabuqina chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "kabuqina chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "kabuqina --yolo bypasses all dangerous command approval prompts for the entire session.",
    "kabuqina chat --source telegram tags the session for filtering in kabuqina sessions list.",
    "kabuqina -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "kabuqina doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "kabuqina dump outputs a compact setup summary — great for bug reports.",
    "kabuqina config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "kabuqina config edit opens config.yaml in your default editor.",
    "kabuqina config check scans for missing or stale configuration options.",
    "kabuqina sessions browse opens an interactive session picker with search.",
    "kabuqina sessions stats shows session counts by platform and database size.",
    "kabuqina sessions prune --older-than 30 cleans up old sessions.",
    "kabuqina skills search react --source skills-sh searches the skills.sh public directory.",
    "kabuqina skills check scans installed hub skills for upstream updates.",
    "kabuqina skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "kabuqina skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "kabuqina mcp add github --command npx adds MCP servers from the command line.",
    "kabuqina mcp serve runs Kabuqina itself as an MCP server for other agents.",
    "kabuqina auth add lets you add multiple API keys for credential pool rotation.",
    "kabuqina completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "kabuqina logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "kabuqina backup creates a zip backup of your entire Kabuqina home directory.",
    "kabuqina profile create coder creates an isolated profile that becomes its own command.",
    "kabuqina profile create work --clone copies your current config and keys to a new profile.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as kabuqina -w).",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Kabuqina tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'kabuqina profile create coder' creates the 'coder' command.",
    "kabuqina profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Kabuqina prints a resume command with session ID and stats.",
    "kabuqina sessions export backup.jsonl exports all sessions for backup or analysis.",
    "kabuqina -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "kabuqina skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .kabuqina/plans/ in the active workspace.",
    ...
    # --- Gateway & Messaging ---
    "Kabuqina runs on 18 platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, email, and more.",
    ...
    "SOUL.md at ~/.kabuqina/SOUL.md is the agent's primary identity — customize it to shape behavior.",
    ...
    "MCP servers auto-generate toolsets at runtime — kabuqina tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.kabuqina/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "kabuqina chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "kabuqina plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "kabuqina acp runs Kabuqina as an ACP server for VS Code, Zed, and JetBrains integration.",
    ...
    "kabuqina status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.kabuqina/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Kabuqina your own.",
    "SOUL.md is auto-seeded with a default personality on first run. Edit ~/.kabuqina/SOUL.md to customize.",
    ...
    "Before a gateway session reset, Kabuqina auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.kabuqina/skills/.hub/audit.log tracks every install and removal operation.",
    ...
    "Every interrupt during an agent run is logged to ~/.kabuqina/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'kabuqina profile create ls' would be rejected.",
    "kabuqina profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "kabuqina claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via kabuqina tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.kabuqina/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.kabuqina/skills/.hub/quarantine/ holds skills pending security review.",
]
