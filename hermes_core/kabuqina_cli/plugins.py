"""
Kabuqina Plugin System
====================

Discovers, loads, and manages plugins from four sources:

1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with kabuqina-agent;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.kabuqina/plugins/<name>/``
3. **Project plugins** – ``./.kabuqina/plugins/<name>/`` (opt-in via
...
class PluginManifest:
    ...
    #              in ~/.kabuqina/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
    # Registry key — path-derived, used by ``plugins.enabled``/``disabled``
    # lookups and by ``kabuqina plugins list``. For a flat plugin at
...
class PluginContext:
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``kabuqina honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``kabuqina <subcommand>``
        ...
        try:
            from kabuqina_cli.commands import resolve_command
    ...
    def register_skill(
        ...
        ``~/.kabuqina/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        #
        # Repo-shipped plugins live next to kabuqina_cli/. Two layouts are
        ...
        # 3. Project plugins (canonical path, then one-release legacy path).
        project_plugins_enabled = (
            _env_enabled("KABUQINA_ENABLE_PROJECT_PLUGINS")
            if "KABUQINA_ENABLE_PROJECT_PLUGINS" in os.environ
            else _env_enabled("HERMES_ENABLE_PROJECT_PLUGINS")
        )
        if project_plugins_enabled:
            current_project_dir = Path.cwd() / ".kabuqina" / "plugins"
            legacy_project_dir = Path.cwd() / ".hermes" / "plugins"
            project_dir = (
                legacy_project_dir
                if legacy_project_dir.exists() and not current_project_dir.exists()
                else current_project_dir
            )
        ...
        for manifest in winners.values():
            ...
            # Built-in backends auto-load — they ship with Kabuqina and must
            # just work. Selection among them (e.g. which image_gen backend
            # services calls) is driven by ``<category>.provider`` config,
            # enforced by the tool wrapper.
            #
            # Bundled platform plugins (gateway adapters like IRC) auto-load
            # for the same reason: every platform Kabuqina ships must be
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `kabuqina plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``kabuqina tools`` TUI so plugin-provided toolsets appear
