...
Add, remove, or reorder entries here — both `kabuqina setup` and
Kabuqina provider selection will pick up the change automatically.
"""
...
_KABUQINA_USER_AGENT = f"kabuqina-cli/{_KABUQINA_VERSION}"
...
def _xai_curated_models() -> list[str]:
    ...
    when the cache is empty or unreadable. Kabuqina refreshes the cache from
    https://models.dev/api.json on normal use, so this list self-heals as
    xAI renames models.

    Mirrors the curated-list role used by direct providers.
    """
    try:
...
def check_nous_free_tier() -> bool:
    ...
    try:
        from kabuqina_cli.auth import get_provider_auth_state
        from providers.nous_auth import resolve_nous_runtime_credentials
...
# this list:  kabuqina model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `kabuqina model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `kabuqina model` TUI
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model a user would be offered first in the ``kabuqina model`` picker.
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    kabuqina-agent is tool-calling-first — every provider path assumes the model
    ...
    **Permissive when the field is missing.** Some OpenRouter-compatible gateways
    ...
    Ported from upstream issue #9068.
    """
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from kabuqina_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — kabuqina-agent
        # requires it and surfacing them leads to immediate runtime failures
        # when the user selects them. Ported from upstream issue #9068.
...
def get_curated_nous_model_ids() -> list[str]:
    ...
    """
    try:
        from kabuqina_cli.model_catalog import get_curated_nous_models
...
def fetch_models_with_pricing(
    ...
    *,
    ...
    headers: dict[str, str] = {
        "Accept": "application/json",
        "User-Agent": _KABUQINA_USER_AGENT,
    }
    if api_key:
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``kabuqina model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from kabuqina_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def curated_models_for_provider(
    ...
    *,
    ...
    # Try live API first (Nous and direct providers that expose /models).
...
def _resolve_static_model_alias(
    ...
    try:
        from kabuqina_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Kabuqina' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``kabuqina_cli.auth.resolve_provider()`` to resolve it to a concrete
...
# ignored by non-OpenAI endpoints, so false positives are harmless.
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Kabuqina should expose the /fast toggle for this model."""
...
# branches below (anthropic, custom, stepfun) — those paths handle freshness
# themselves.
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    Tries live API endpoints for providers that support them (Nous, etc.),
    falling back to static lists. For providers in ``_MODELS_DEV_PREFERRED``
    (xiaomi, deepseek, smaller inference providers, etc.),
    models.dev entries are merged on top of curated so new models released
    on the platform appear in ``/model`` without a Kabuqina release.
    """
    normalized = normalize_provider(provider)
    if normalized == "openrouter":
    ...
    if normalized == "nous":
        ...
        try:
            from providers.nous_auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
    ...
    if normalized == "stepfun":
        try:
            from kabuqina_cli.auth import resolve_api_key_provider_credentials
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _KABUQINA_USER_AGENT}
...
def probe_api_models(
    ...
    tried: list[str] = []
    headers: dict[str, str] = {"User-Agent": _KABUQINA_USER_AGENT}
    ...
    *,
    ...
    if normalized == "custom":
        ...
        message = (
            ...
            f"Kabuqina will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so Kabuqina cannot verify the model name."
    ...
    # validate against the curated list from provider_model_ids(). Without
    # this block, validate_requested_model would reject every model on such
    # providers, switch_model() would return success=False, and the gateway
    # would never write to _session_model_overrides.
