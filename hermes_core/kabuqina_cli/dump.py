...
from kabuqina_cli.config import get_kabuqina_home, get_env_path, get_project_root, load_config
...
def _redact(value: str) -> str:
    ...
    ``kabuqina dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _count_skills(kabuqina_home: Path) -> int:
...
def _cron_summary(kabuqina_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = kabuqina_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    try:
        from kabuqina_cli import __version__, __release_date__
    except ImportError:
        __version__ = "(unknown)"
        __release_date__ = ""
    ...
    lines.append(f"kabuqina_home:    {display_kabuqina_home()}")
    ...
    toolsets = config.get("toolsets", ["kabuqina-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(kabuqina_home)}")
    lines.append(f"  skills:             {_count_skills(kabuqina_home)}")
