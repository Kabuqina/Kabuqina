...
"""Custom (user-defined) provider config helpers for kabuqina_cli.

Extracted from ``kabuqina_cli/config.py``: normalization of ``providers:`` config
entries and selection of compatible custom providers / their context lengths.
``kabuqina_cli.config`` re-exports every name.
"""
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Kabuqina versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``kabuqina_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
