"""
Unified tool configuration for Kabuqina.

`kabuqina tools` and `kabuqina setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.kabuqina/config.yaml under
...
from kabuqina_cli.nous_subscription import (
...
from kabuqina_cli.cli_output import (  # noqa: E402 — late import block
...
def _get_effective_configurable_toolsets():
    ...
    already appears in ``CONFIGURABLE_TOOLSETS`` is skipped, and we want the
    built-in label/description to win.
    Without the dedupe, ``kabuqina tools`` → "reconfigure existing" would
    list the same toolset twice.
    """
    result = list(CONFIGURABLE_TOOLSETS)
    seen = {ts_key for ts_key, _, _ in result}
    try:
        from kabuqina_cli.plugins import discover_plugins, get_plugin_toolsets
...
from kabuqina_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
    ...
    if post_setup_key in ("agent_browser", "browserbase"):
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_kabuqina_home()}/hermes-agent && npm install")
                if result.stderr:
                    _print_info(f"      {result.stderr.strip()[:200]}")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in kabuqina-agent directory)")
            return
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.kabuqina/config.yaml")
        ...
        try:
            from kabuqina_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
            ...
            _print_info("    Run manually: kabuqina plugins enable observability/langfuse")
        _print_info("    Restart Kabuqina for tracing to take effect.")
        _print_info("    Verify: kabuqina plugins list")
...
def _get_platform_tools(
    ...
    *,
    ...
    # "kabuqina-cli" (which include all _KABUQINA_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # (e.g. "kabuqina-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
    ...
    skip |= {k for k in TOOLSETS if k.startswith("kabuqina-")}
    ...
    # A plugin toolset is "known" for a platform once `kabuqina tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by kabuqina tools — default enabled
                enabled_toolsets.add(pts)
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (kabuqina-cli, kabuqina-telegram, etc.)
    ...
    # Opening `kabuqina tools` is the user's opt-in to reconfigure tools, so treat
...
def _prompt_choice(question: str, choices: list, default: int = 0) -> int:
    ...
    from kabuqina_cli.curses_ui import curses_radiolist
...
def _prompt_toolset_checklist(platform_label: str, enabled: Set[str], platform: str = "cli") -> Set[str]:
    ...
    from kabuqina_cli.curses_ui import curses_checklist
...
def _plugin_image_gen_providers() -> list[dict]:
    ...
    try:
        ...
        from kabuqina_cli.plugins import _ensure_plugins_discovered
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `kabuqina tools` and `kabuqina setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚕ Kabuqina Tool Configuration", Colors.CYAN, Colors.BOLD))
    ...
    print()
    from kabuqina_constants import display_kabuqina_home
    print(color(f"  Tool configuration saved to {display_kabuqina_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'kabuqina' or gateway restart.", Colors.DIM))
    print()
