...
from kabuqina_cli import auth as auth_mod
from providers.credential_pool import CredentialPool, PooledCredential, get_custom_provider_pool_key, load_pool
...
from kabuqina_cli.config import get_compatible_custom_providers, load_config
...
def _get_named_custom_provider(requested_provider: str) -> Optional[Dict[str, Any]]:
        ...
        for ep_name, entry in providers.items():
            ...
            if requested_norm in {ep_name, name_norm, f"custom:{name_norm}"}:
                ...
                if base_url:
                    ...
                    # already does, so without this lift migrated custom
                    # Anthropic-compatible providers silently downgrade to
    ...
    if isinstance(custom_providers, dict):
        logger.warning(
            ...
            "Run 'kabuqina doctor' for details."
        )
        return None
...
def _resolve_explicit_runtime(
    *,
    ...
    if provider == "anthropic":
        ...
        if not api_key:
            from providers.anthropic import resolve_anthropic_token
    ...
    if pconfig and pconfig.auth_type == "api_key":
        ...
        if configured_mode:
            ...
            # Auto-detect from URL (Anthropic /anthropic suffix, Kimi /coding).
            detected = _detect_api_mode_for_url(base_url)
            if detected:
                api_mode = detected
...
def resolve_runtime_provider(
    *,
    ...
    if pool and pool.has_credentials():
        ...
        # non-runtime contexts like `kabuqina auth list`).  If the key is
    ...
    if provider == "anthropic":
        ...
        if _is_azure_endpoint:
            ...
            # chain.  Accept both `key_env` (Kabuqina canonical — matches the
            # custom_providers field name) and `api_key_env` (read by many
            # Kabuqina-compatible importers).
            # Matches config.yaml examples for Azure-hosted Anthropic endpoints.
            token = ""
