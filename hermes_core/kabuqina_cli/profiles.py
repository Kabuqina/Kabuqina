"""
Profile management for multiple isolated Kabuqina instances.

Each profile is a fully independent KABUQINA_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.kabuqina/profiles/<name>/`` by default.

The "default" profile is ``~/.kabuqina`` itself — backward compatible,
zero migration needed.

Usage::

    kabuqina profile create coder          # fresh profile + bundled skills
    kabuqina profile create coder --clone  # also copy config, .env, SOUL.md
    kabuqina profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    kabuqina -p coder chat               # or via flag
    kabuqina profile use coder             # set as sticky default
    kabuqina profile delete coder          # remove profile + alias
"""
...
_PROFILE_DIRS = [
    ...
    # See kabuqina_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    ``~/.kabuqina`` contains ``profiles/<name>/`` for sibling named profiles.
...
# Directories/files to exclude when exporting the default (~/.kabuqina) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "kabuqina-agent",         # canonical repo checkout (multi-GB)
    "hermes-agent",           # one-release legacy repo checkout
    ...
    "errors.log",
    ".kabuqina_history", ".hermes_history",  # current + one-release legacy
...
_RESERVED_NAMES = frozenset({
    "kabuqina", "hermes", "default", "test", "tmp", "root", "sudo",
})

# Kabuqina subcommands that cannot be used as profile names/aliases
_KABUQINA_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the Kabuqina root, NOT to the current KABUQINA_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where KABUQINA_HOME points outside
    ``~/.kabuqina``, profiles live under ``KABUQINA_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_kabuqina_home() / "profiles"


def _get_default_kabuqina_home() -> Path:
    """Return the default (pre-profile) KABUQINA_HOME path.

    In standard deployments this is ``~/.kabuqina``.
    In Docker/custom deployments where KABUQINA_HOME is outside ``~/.kabuqina``
    (e.g. ``/opt/data``), returns KABUQINA_HOME directly.
    """
    from kabuqina_constants import get_default_kabuqina_root
    return get_default_kabuqina_root()


# One-release compatibility for plugins importing the former private helper.
_get_default_hermes_home = _get_default_kabuqina_home


def _get_active_profile_path() -> Path:
    """Return the path to the sticky active_profile file."""
    return _get_default_kabuqina_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name == "default":
        return  # special alias for ~/.kabuqina
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its KABUQINA_HOME directory."""
    if name == "default":
        return _get_default_kabuqina_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, Kabuqina subcommands, existing binaries in PATH.
    """
    if name in _RESERVED_NAMES:
        return f"'{name}' is a reserved name"
    if name in _KABUQINA_SUBCOMMANDS:
        return f"'{name}' conflicts with a kabuqina subcommand"
        ...
        if result.returncode == 0:
            ...
            if existing_path == str(wrapper_dir / name):
                try:
                    ...
                    if "kabuqina -p" in content or "hermes -p" in content:
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_kabuqina_home()
...
def create_profile(
    ...
    if name == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.kabuqina)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.kabuqina/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches KABUQINA_HOME at module level.
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if name == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.kabuqina).\n"
            "To remove everything, use: kabuqina uninstall"
        )
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.kabuqina/active_profile``. Use ``"default"`` to clear.
    """
    validate_profile_name(name)
    if name != "default" and not profile_exists(name):
        raise FileNotFoundError(
            f"Profile '{name}' does not exist. "
            f"Create it with: kabuqina profile create {name}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from KABUQINA_HOME.

    Returns ``"default"`` if KABUQINA_HOME is not set or points to ``~/.kabuqina``.
    Returns the profile name if KABUQINA_HOME points into ``~/.kabuqina/profiles/<name>``.
    Returns ``"custom"`` if KABUQINA_HOME is set to an unrecognized path.
    """
    from kabuqina_constants import get_kabuqina_home
    kabuqina_home = get_kabuqina_home()
    resolved = kabuqina_home.resolve()

    default_resolved = _get_default_kabuqina_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if name == "default":
        # The default profile IS ~/.kabuqina itself — its parent is ~/ and its
        # directory name is ".kabuqina", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: kabuqina profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.kabuqina itself — disallow
    # that and guide the user toward a named profile.
    if inferred_name == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.kabuqina). "
            "Specify a different name: kabuqina profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="kabuqina_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    candidates = [
        new_dir / "honcho.json",
        _get_default_kabuqina_home() / "honcho.json",
...
def generate_bash_completion() -> str:
    """Generate a bash completion script for kabuqina profile names."""
    return '''# Kabuqina profile completion
# Add to ~/.bashrc: eval "$(kabuqina completion bash)"

_kabuqina_profiles() {
    local profiles_dir="${KABUQINA_HOME:-$HOME/.kabuqina}/profiles"
...
_kabuqina_completion() {
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_kabuqina_profiles)" -- "$cur"))
        return
    fi
...
complete -F _kabuqina_completion kabuqina
'''


def generate_zsh_completion() -> str:
    """Generate a zsh completion script for kabuqina profile names."""
    return '''#compdef kabuqina
# Kabuqina profile completion
# Add to ~/.zshrc: eval "$(kabuqina completion zsh)"
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a KABUQINA_HOME path string.

    Called early in the CLI entry point, before any Kabuqina modules
    are imported, to set the KABUQINA_HOME environment variable.
    """
    ...
    if profile_name != "default" and not profile_dir.is_dir():
        raise FileNotFoundError(
            ...
            f"Create it with: kabuqina profile create {profile_name}"
        )
