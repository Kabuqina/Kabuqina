"""CLI entry point for the kabuqina-agent ACP adapter.

Loads environment variables from ``~/.kabuqina/.env``, configures logging
...
def _load_env() -> None:
    """Load .env from KABUQINA_HOME (default ``~/.kabuqina``)."""
    from kabuqina_cli.env_loader import load_kabuqina_dotenv

    kabuqina_home = get_kabuqina_home()
    loaded = load_kabuqina_dotenv(kabuqina_home=kabuqina_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", kabuqina_home / ".env"
        )
...
def main() -> None:
    ...
    logger.info("Starting kabuqina-agent ACP adapter")
    ...
    import acp
    from .server import KabuqinaACPAgent
