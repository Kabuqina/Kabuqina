# Contributing to Kabuqina Agent Core

Thank you for contributing to Kabuqina's owned agent core. This guide covers the local development workflow and retained core architecture.
...
2. **Windows desktop compatibility** — the shipped Kabuqina product is Windows-only; keep portable core code where practical.
...
Bundled skills (in `skills/`) ship with every Kabuqina install. They should be **broadly useful to most users**:

- Document handling, web research, common dev workflows, system administration
- Used regularly by a wide range of people

If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `kabuqina skills browse` (labeled "official") and install it with `kabuqina skills install` (no third-party warning, builtin trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Nous Research Discord](https://discord.gg/NousResearch). Users can install it with `kabuqina skills install`.
...
```bash
mkdir -p ~/.kabuqina/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.kabuqina/config.yaml
touch ~/.kabuqina/.env

# Add at minimum an LLM provider key:
echo "OPENROUTER_API_KEY=***" >> ~/.kabuqina/.env
```
...
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/kabuqina" ~/.local/bin/kabuqina

# Verify
kabuqina doctor
kabuqina chat -q "Hello"
```
...
├── toolsets.py               # Tool groupings and presets (kabuqina-cli, kabuqina-telegram, etc.)
├── kabuqina_state.py         # SQLite session database with FTS5 full-text search, session titles
...
│
├── kabuqina_cli/               # CLI command implementations
...
│
├── skills/                   # Bundled skills (copied to ~/.kabuqina/skills/ on install)
├── optional-skills/          # Official optional skills (discoverable via hub, not activated by default)
├── environments/             # RL training environments (Atropos integration)
├── tests/                    # Test suite
├── website/                  # Documentation site (hermes-agent.nousresearch.com)
│
├── cli-config.yaml.example   # Example configuration (copied to ~/.kabuqina/config.yaml)
└── AGENTS.md                 # Development guide for AI coding assistants
```

### User configuration (stored in `~/.kabuqina/`)

| Path | Purpose |
|------|---------|
| `~/.kabuqina/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.kabuqina/.env` | API keys and secrets |
| `~/.kabuqina/auth.json` | OAuth credentials (Nous Portal) |
| `~/.kabuqina/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.kabuqina/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.kabuqina/state.db` | SQLite session database |
| `~/.kabuqina/sessions/` | JSON session logs |
| `~/.kabuqina/cron/` | Scheduled job data |
| `~/.kabuqina/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`kabuqina_state.py`) with full-text search and unique session titles. JSON logs go to `~/.kabuqina/sessions/`.
...
The user may skip setup and keep loading the skill. Kabuqina only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `kabuqina setup` or update `~/.kabuqina/.env` locally.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing Kabuqina tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `kabuqina --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

Kabuqina uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.kabuqina/skins/<name>.yaml`:
...
Add to `_BUILTIN_SKINS` dict in `kabuqina_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `kabuqina_cli/skin_engine.py` for the full schema and existing skins as examples.

---

## Cross-Platform Compatibility

Kabuqina runs on Linux, macOS, and WSL2 on Windows. When writing code that touches the OS:
...
Kabuqina has terminal access. Security matters.
...
2. **Test manually**: Run `kabuqina` and exercise the code path you changed
...
- Include: OS, Python version, Kabuqina version (`kabuqina version`), full error traceback
