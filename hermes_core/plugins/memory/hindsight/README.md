...
echo "HINDSIGHT_API_KEY=your-key" >> ~/.kabuqina/.env
```
...
Daemon startup logs: `~/.kabuqina/logs/hindsight-embed.log`
...
Config file: `~/.kabuqina/hindsight/config.json`
...
The LLM API key is stored in `~/.kabuqina/.env` as `HINDSIGHT_LLM_API_KEY`.
