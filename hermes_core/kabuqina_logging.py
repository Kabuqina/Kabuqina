"""Centralized logging setup for Kabuqina's owned agent core.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.kabuqina/logs/`` (profile-aware via ``get_kabuqina_home()``).
...
from kabuqina_constants import get_config_path, get_kabuqina_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_kabuqina_session_injector", False):
    ...
    _session_record_factory._kabuqina_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``kabuqina logs --component``.
COMPONENT_PREFIXES = {
    "gateway": ("gateway",),
    "agent": ("agent", "run_agent", "model_tools"),
    "tools": ("tools",),
    "cli": ("kabuqina_cli", "cli"),
    "cron": ("cron",),
}
...
def setup_logging(
    *,
    kabuqina_home: Optional[Path] = None,
    ...
    """Configure the Kabuqina logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    kabuqina_home
        Override for the Kabuqina home directory. Falls back to
        ``get_kabuqina_home()`` (profile-aware).
    hermes_home
        Deprecated one-release alias for ``kabuqina_home``.
    log_level
    ...
    home = kabuqina_home or hermes_home or get_kabuqina_home()
    log_dir = home / "logs"
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_kabuqina_verbose", False):
                return

    handler = logging.StreamHandler()
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._kabuqina_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    inherit the Kabuqina group. However, both _open() (initial creation) and
    ...
    def __init__(self, *args, **kwargs):
        from kabuqina_cli.config import is_managed
