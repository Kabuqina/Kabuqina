...
function Probe() {
  ...
  return <><output data-testid="draft-items">{data?.items.map((item) => item.title).join(",") ?? "loading"}</output><button type="button" onClick={() => { void drafts.activate("draft-a"); }}>activate</button></>;
}
