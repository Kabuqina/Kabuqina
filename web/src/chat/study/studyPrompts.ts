...
export const LEARNING_PATH_PROMPT = [
  ...
  "如果已有最近一次 evaluation 或测验结果，请读取其中的薄弱点、证据和建议；动态重规划时只调整未完成阶段，已完成阶段保持不变，并用一句话说明重排依据。",
  "当信息足够、或我说“可以了”时，请使用 STUDY learning 工具创建或更新 kind=learning_plan 草稿。payload 至少包含 phases 数组；每个 phase 包含 title、tasks，tasks 每项包含 title，可选 order 与 done_when；可补充 phase.status(pending/active/done) 和 focus(对应画像或评估薄弱点)。创建后只简短告诉我已生成，待我在“学习路径”里审核激活。",
...
export const LEARNING_EVALUATION_PROMPT = [
  ...
  "当评估证据足够时，请使用 STUDY learning 工具创建 kind=evaluation 草稿。payload 必须包含 observations 数组；可包含 weak_points 与 suggestions 字符串数组。observations 必须写明证据来源，不要把推断写成事实。创建后只简短告诉我已生成，待我在“学习评估闭环”里审核激活。",
