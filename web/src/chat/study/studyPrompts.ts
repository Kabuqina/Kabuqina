...
export const LEARNING_RESOURCE_PACK_PROMPT = [
  ...
  "输出格式请固定为：1. 资源包摘要；2. 多智能体协同过程；3. 资源清单总览；4. 各类资源正文；5. 使用顺序建议；6. 质量检查与不确定信息；7. 后续可扩展资源。",
...
export const LEARNING_TUTOR_PROMPT = [
  ...
  "输出格式请固定为：1. 问题复述与已知条件；2. 个性化解答；3. 图解/导图大纲；4. 可选视频/动画脚本；5. 练习巩固；6. 仍需确认的信息。",
...
export const LEARNING_EVALUATION_PROMPT = [
  ...
  "输出格式请固定为：1. 评估摘要；2. 维度评分表；3. 证据与不确定信息；4. 画像更新建议；5. 学习路径调整；6. 资源推送策略；7. 下次复测方案。",
