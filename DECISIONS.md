...
| Tutor runtime L-0 contract (2026-07-19) | Structured Tutor is an explicit desktop learning activity, never inferred from ordinary Chat. It uses a separate `TutorGraphEngine` and versioned state without changing the stateless ordinary Agent graph. Durable identity is host-injected `owner + space + activity_kind + activity_id`; `learning.db` gains separate activity-run/checkpoint tables with atomic revision CAS, while the existing `learning_activities` table remains an append-only event log. Learner checks and tool approvals have separate types/IDs/stores/APIs. The lifecycle includes recoverable `interrupted`; terminal checkpoints are cleared for `completed/blocked/cancelled`. Every segment has hard node/call/token/time budgets, and Gateway Tutor remains out of v0.5 scope. Full contract: `docs/superpowers/plans/2026-07-19-v0.5.0-ctl-b01-tutor-runtime-design.md`. |
