...
| Knowledge graph projection (2026-07-19) | Course graphs are deterministic, read-only projections of owner/space-scoped **active** `knowledge_base` artifacts. Draft or rejected knowledge never appears as fact. Concepts may declare exact-name `prerequisites` (directed prerequisite → dependent) and symmetric `related` links; Web layout and node positions are presentation state only. Concept documents use the same active-only boundary. |
