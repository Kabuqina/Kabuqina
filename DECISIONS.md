...
| D5 legacy Web retirement (2026-07-16) | `/study/:spaceId/:page` is the only Study lifecycle UI. Retire `StudySection` and dead flashcard/quiz localStorage stores/helpers, while retaining Python migration diagnostics/export for rollback. The old profile key has not completed its release-window gate, so its only remaining reader is an isolated one-shot adapter: failure preserves the old value byte-for-byte and a confirmed backend migration clears it. Final removal waits for post-A-R3 upgrade evidence. |
