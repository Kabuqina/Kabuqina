...
| Knowledge-source deletion (2026-07-31) | Deleting an imported source is a non-cascading soft delete: the `resource_pack` artifact transitions from `active` to the explicit terminal status `deleted`. Its stable id, envelope and source path record remain as a tombstone; plans, knowledge cores, generated learning/practice/evaluation artifacts, items and activities are never deleted or nulled. Source resolution reports `deleted` instead of `null`. The desktop operation removes only the source from active Study surfaces and never deletes the user's original filesystem file. |
