...
const HTML_DIR = path.join(ROOT, "editorial-forest__html");
...
const BUILD_ROOT = path.resolve("outputs/manual-20260608-editorial-forest/presentations/html-background-master");
...
async function writeSlideModules(slides) {
  ...
  await ctx.addImage(slide, {
    ...
    fit: "cover",
    alt: "Editorial Forest HTML-rendered background ${slideNo}",
...
async function main() {
  ...
  await fs.writeFile(
    ...
    JSON.stringify(
      {
        version: 1,
        visual_master: "editorial_forest",
